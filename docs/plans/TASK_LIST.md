...
- [ ] **Gemini会話ログのインポート機能** <!-- id: new -->
  - 詳細: Geminiアプリの共有リンクは直接スクレイピングできないため、ユーザーが保存したHTMLファイルから会話ログを取り込む機能を実装する。UIにファイルアップロードを追加し、BeautifulSoupで解析・取り込みを行う。
  - 優先度: 🔴高
...
- [x] **OpenAI互換モデル設定の永続化不具合修正** <!-- id: 808 -->
  ...
  - 優先度: 🔴高
- [x] **睡眠時記憶整理のエラー修正** (High) [レポート](../reports/2026-02-04_Fix_Sleep_Memory_and_RAG_Log.md)
