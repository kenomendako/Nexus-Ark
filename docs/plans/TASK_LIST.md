...
- [x] **[BUG] ポモドーロタイマーの通知テーマと残り時間表示の修正** <!-- id: 815 --> ([Report](../reports/2026-02-09_Pomodoro_Fix_and_Log_Repair.md))
  - 詳細: 作業終了時に休憩への入り、休憩終了時に作業への戻りの通知が正しく行われるようテーマ順序を修正し、残り時間計算のロジックを改善。ログファイル修復も実施。
  - 優先度: 🟡中
...
- [x] **お出かけ機能：過去ログエクスポート用タグの追加と帰宅時の重複除去** <!-- id: 814 -->
  - 詳細: エクスポート時に過去ログを特定のタグ（XML形式等）で囲むオプションを追加。帰宅インポート時にそのタグ内を自動除去し、重複を防ぐ。
  - レポート: [レポート](../reports/2026-02-09_Outing_Feature_Enhancements.md)
  - 優先度: 🔴高
