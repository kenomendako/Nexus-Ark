...
- **お出かけ機能のログ重複防止と言語サニタイズ**: 過去ログおよびエピソード記憶のエクスポート時に AI のメタタグ（表情、感情、トレース）と思考ログを除去する機能を実装。さらに過去ログをタグで囲むことで、帰宅（インポート）時の自動重複除去を実現。 (2026-02-09)
- **オンボーディング＆マイグレーション完成**: 新規インストールと移行の選択、データ引継ぎ、オリヴェRAG索引の自動更新、完了後の自動リロードを実装。 (2026-02-08)
- **既存ユーザーのマイグレーションツールの実装**: v0.1.0 (Beta) からのデータ（設定、キャラクター、ログ）を安全に引き継ぎつつ、オリヴェのみ最新アセットをマージする「二段階移行ロジック」を搭載した `tools/migrate_from_old.py` とバージョン判定基盤を実装。 (2026-02-08)
- **サンプルペルソナ（オリヴェ）の整備**: 情景描写テキストの文体修正（記述体化）と全8箇所の高品質画像アセットの生成・同梱完了。 (2026-02-08)
- **仕様書の大幅更新**: `NEXUS_ARK_SPECIFICATION.md` を現行実装に合わせ約250行→480行超に拡充。約30の未記載機能を追記し、ユーザーフィードバック10項目を反映。 (2026-02-07)
- **お出かけ機能のログ重複防止と言語サニタイズ**: エクスポート時に過去ログを `<nexus_ark_past_logs>` タグで囲む機能を追加し、重複インポートを自動防止。AIメタタグと思考ログの除去と併せて、外部連携の利便性を大幅に向上。 (2026-02-09)
- **README刷新と配布構成の最終化**: GitHub用/配布用READMEを統合し、トーンを「Grounded & User-Centric」に統一。配布パッケージのルートにREADMEと仕様書を配置し、簡易版READMEを廃止してユーザビリティを向上。Mac/Linux免責事項も追加。 (2026-02-09)
- **ルーム状態混線と保存通知の修正**: モバイル等での混線（State/UI不一致）をFrontend Bindingにより修正。保存通知の消失と過剰反応を解消。 (2026-02-06)
...
- **Pinokio 実機テスト (🚫中止)**: 初期導入コストの高さから非推奨化。軽量ランチャー開発へシフト。 ([Report](reports/2026-02-08_Pinokio_Test_Cancelled.md))

### Fixed / Implemented
| 2026-02-09 | **README刷新と配布構成の最終化**<br>・GitHub/配布用READMEの統合とトーン統一<br>・配布パッケージルートへのドキュメント配置<br>・Mac/Linux免責事項の追加 | [レポート](reports/2026-02-09_README_Refinement_Completion.md) |
| 2026-02-09 | **ポモドーロタイマー修正とログファイル修復**<br>・作業/休憩の通知メッセージ逆転と残り時間計算を修正<br>・ルシアンのログファイルから数千行の異常メタタグを除去 | [レポート](reports/2026-02-09_Pomodoro_Fix_and_Log_Repair.md) |
...
| 2026-02-06 | **ルーム状態混線と保存通知の修正**<br>・モバイル等での混線（State/UI不一致）をFrontend Bindingにより修正<br>・保存通知の消失と過剰反応（ノイズ）をGrace Period導入により適正化 | [レポート](reports/2026-02-06_Fix_Room_State_and_Notifications.md) |
