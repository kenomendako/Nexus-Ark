|- **最終更新:** 2026-02-03 (システムプロンプトの最適化と削除機能のバグ修正)
- **現在のフェーズ:** Phase 3 (知識・分析機能の強化 / 最適化)

## [Unreleased]
### Added
- [Phase 2] ペンディングシステムメッセージ（影の僕からの提案）用の独立したプロンプトプレースホルダー `{pending_messages_section}` を追加。

### Fixed
- `utils.py`: メッセージ削除時に `new_messages` が定義されていないために発生していた `NameError` を修正。

### Changed
- システムプロンプトから「記憶しているエンティティ一覧」を削除（トークン節約のため）。
...
1. **システムプロンプトの最適化（エンティティ一覧の削除）**: (次)
2. **配布基盤構築**: Pinokio/uv を前提としたモダンな配布フローの実装
3. **Webhook管理UIの整合性確認**: UIと内部状態の一致
