...
- **Gemini:** 共有URLを入力することでインポート可能（Playwrightによるスクレイピング）。PCとモバイルの両方の共有URLに対応。
...
- **Google Gemini:** デフォルトのプロバイダ。`gemini-2.5-flash`などのモデルを使用。
...
- **表示:** チャット後のトークン表示の隣に、そのセッションのArousalスコアが表示されます。
...
- **使い分け:**「全ログ」設定では古いログを要約、「本日分」設定ではエピソード記憶で補完
...
*この仕様書はNexus Ark v1.0 (2026-02) の機能を反映しています。*