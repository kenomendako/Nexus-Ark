...
## [0.2.1] - 2026-02-09
- **README刷新と配布構成の最終化**: GitHub用/配布用READMEを統合し、トーンを「Grounded & User-Centric」に統一。配布パッケージのルートにREADMEと仕様書を配置し、レスポンシブメニュー（...）の注釈を追加。
- **ペルソナ重複（オリヴェ増殖）の修正**: 移行時に作成されるバックアップフォルダがUIに表示されないよう、ドット（`.`）で始まる隠しフォルダの読み込みをスキップするロジックを実装。また、移行時の退避先を `characters/` 外の `backups/migration_retired/` に変更し、UIへの干渉を完全に排除。 (2026-02-10)
- **お出かけログ構成の高度化と帰宅インポートUXの改善**: 「本日分のみ」のエクスポート、文字数閾値ベースの自動要約、インポート前のプレビュー・編集機能、システムマーカー（開始・終了通知）の表示制御を実装。Gemini URL取込時の取得不具合も修正。 (2026-02-09)
- **重複防止・過去ログ除去ロジックの強化**: エクスポート時に付与される「## 直近の会話ログ」見出しとタグのセットを、正規表現により確実に除去。Gemini 貼り付け時の改行揺らぎにも対応。 (2026-02-09)
...
### Fixed
- **起動エラーの解消**: 変数名変更の漏れ（`NameError: gemini_import_button`）を修正。 (2026-02-09)
- **インポート時エラーの解消**: Gemini URL からの取得時に発生していた引数不整合（`AttributeError`）を修正。 (2026-02-09)
- **ポモドーロタイマーの不具合修正**: 作業/休憩終了時のメッセージ逆転と残り時間計算ロジックを修正。 (2026-02-09)
- **ログファイル修復**: ルシアンのログファイルに発生していたメタタグ異常増殖を一括削除し修復。 (2026-02-09)
- **Wiring検証の改善**: `validate_wiring.py` の誤検出ロジックを修正し、ジェネレータ関数に対応。 (2026-02-09)
...
- **APIキーローテーションの信頼性向上 (2026-02-01):** APIキーの枯渇状態をディスクに永続化し、プロセス間での共有を実現。`DreamingManager`（睡眠モード）に自動ローテーション・リトライロジックを追加し、バックグラウンド処理の堅牢性を強化。ローテーション後のキー名保存漏れやタイポも修正。[レポート](docs/reports/2026-02-01_api_key_rotation_reliability.md)
- **Gemini APIキーの優先順位と内部処理設定の修正 (2026-02-01):** 無料キー（Free Keys）を優先的に使用するローテーション・ロジックを実装し、有料キーの不必要な消費を防止。また、検索や要約などの内部処理においてルーム個別の設定ではなくプロジェクト共通のAPIキー設定を強制するように修正。`config.json` の型不整合も解消。[レポート](docs/reports/2026-02-01_gemini_api_fix.md)
- **APIキーローテーションの高速化 (2026-02-01):** 429 RESOURCE_EXHAUSTED 発生時に `agent_node` の冗長なリトライを廃止するように改善。ローテーション待機時間を 10分の1 (0.1s) に短縮し、クォータ制限に達した際の応答遅延を劇的に削減。[レポート](docs/reports/2026-02-01_retry_fix_report.md)
...
- **階層的エピソード記憶圧縮 (2026-01-18):** 日次→週次→月次の3層圧縮を導入。週次圧縮の閾値を60日→3日に短縮、月次圧縮を新規追加。過去1ヶ月選択時の文字数を約7,000文字（従来比1/6）に削減。[レポート](docs/reports/2026-01-18_hierarchical_episodic_compression.md)
...
- **日次要約プロンプトの改善 (2026-01-19):** 序文禁止を明示し、文字数上限を800-1200文字に厳密化。「振り返ると」等のメタ語りを抑制。[レポート](docs/reports/2026-01-19_daily_summary_prompt_fix.md)
...
### Fixed
- **RAG検索時のAPIキーローテーション不具合修正 (2026-02-03):** 429エラー(Resource Exhausted)検知時に自動でAPIキーをローテーションし、検索をリトライするロジックを実装。検索失敗による応答生成中断を防止。[レポート](docs/reports/2026-02-03_rag_key_rotation_fix.md)
- **APIキー枯渇時の救済措置(Rescue Strategy)導入 (2026-02-03):** 全てのAPIキーが枯渇状態とマークされた場合、最も回復の可能性が高い（古く枯渇した）キーを自動的に再試行候補として救済するロジックを追加。一時的な429エラーによるデッドロックを解消。[レポート](docs/reports/2026-02-03_rag_key_rotation_rescue_fix.md)
- **ログ管理画面の検索月フィルタリング改善 (2026-02-03):** キーワード検索ヒット時、該当する月のみをプルダウンに表示し、過去ログ探索を効率化。[レポート](docs/reports/2026-02-03_log_management_ui.md)
...
- **推定トークン数と実送信トークン数の乖離修正 (2026-01-18):** `count_input_tokens`関数が実送信時と異なる履歴構築ロジックを使用していた問題を修正。① `convert_raw_log_to_lc_messages()`と`merge_consecutive_messages()`を使用して履歴構築を統一。② 自動要約シミュレーション時に既存の要約テキストを使用。③ 記憶想起（RAG）、自己意識（目標・動機・未解決の問い）、エンティティ一覧、研究ノート目次、移動可能な場所リストなど、実際に注入されるコンテキストの見積もりを追加。④ トークン表示を「入力(推定) / 実入力 / 実合計」の3項目に変更し、入力のみの推定値と入力＋出力の合計値を区別して表示。
...
- **チャット支援のログ修正で思考ログが消失する問題を修正 (2026-01-21):** 新形式`[THOUGHT]...[/THOUGHT]`タグに対応しておらず、思考ログが消失・修正されない問題を修正。`handle_log_punctuation_correction`と`handle_chatbot_edit`を両形式対応に拡張し、元のタグ形式を維持。[レポート](docs/reports/2026-01-21_thought_log_fix.md)
