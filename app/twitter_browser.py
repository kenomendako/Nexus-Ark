...
async def _post_tweet_async(cookies_path: str, text: str, reply_to_url: Optional[str] = None) -> str:
    """自動投稿を実行する。reply_to_url が指定されている場合はリプライとして投稿する。"""
    if not os.path.exists(cookies_path):
...
def run_post_tweet(cookies_path: str, text: str, reply_to_url: Optional[str] = None) -> str:
    return _run_async_in_new_loop(_post_tweet_async(cookies_path, text, reply_to_url))
