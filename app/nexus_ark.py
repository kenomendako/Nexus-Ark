...
try:
    ...
    #working_memory_editor_code, #memory_txt_editor_code, #notepad_editor_code, #system_prompt_editor, #core_memory_editor_code {
...
""")
                    ...
                    with gr.Tabs() as settings_tabs:
                        with gr.TabItem("共通") as common_settings_tab:
                            with gr.Accordion("🔑 APIキー / Webhook管理", open=False):
                                ...
                                # ローカルLLM (Ollama)
                                with gr.Accordion("ローカルLLM (Ollama推奨)", open=False) as local_llm_group:
                                    gr.Markdown(
                                        "💡 **ローカルLLM**: ローカルでモデルを動かす場合は **Ollama** の利用を推奨します。\n"
                                        "1. <a href='https://ollama.com/' target='_blank' style='color: #4da6ff; text-decoration: underline;'>Ollama公式サイト</a> からインストールし、モデルをダウンロードします（例: `ollama run qwen2.5:3b`）。\n"
                                        "2. 下記のボタンを押すと、Ollama用の接続設定（OpenAI互換プロファイル）が自動追加されます。"
                                    )
                                    add_ollama_profile_button = gr.Button("Ollama用の接続設定を追加（推奨）", variant="primary", size="sm")
                                    gr.Markdown("---", elem_classes=["separator"])
                                    gr.Markdown(
                                        "📜 *(非推奨)* 直接GGUFモデルをロードする場合 (llama-cpp-pythonが必要):"
                                    )
                            ...
                            with gr.Accordion("🔧 内部処理モデル設定", open=False):
                                ...
                                with gr.Row():
                                    delete_inventory_item_button = gr.Button("🗑️ 削除", variant="stop")
                                    copy_inventory_item_button = gr.Button("👯 コピー")
                                    refresh_inventory_button = gr.Button("🔄 更新")
                                ...
                                _provider_choices = [("Google (Gemini)", "google")] + [(name, name) for name in _openai_provider_names] + [("Anthropic (Claude)", "anthropic"), ("ローカル(llama.cpp/GGUF)", "local")]
                                ...
                                with gr.Row():
                                    ...
                                    internal_processing_model = gr.Dropdown(
                                        choices=[
                                            (m + " (Gemini)", m) for m in constants.SUMMARIZATION_MODEL_OPTIONS
                                        ] + [
                                ...
                                with gr.Row():
                                    internal_translation_provider = gr.Dropdown(
                                        choices=_provider_choices,
                                        value=_internal_settings.get("translation_provider", "google"),
                                        label="プロバイダ",
                                        scale=1,
                                        allow_custom_value=True,
                                        interactive=True
                                    ...
                                    internal_translation_model = gr.Dropdown(
                                        choices=[
                                            ...
                                            ("qwen2.5:3b (Ollama推奨)", "qwen2.5:3b"),
                        ...
                        with gr.TabItem("個別") as individual_settings_tab:
                            ...
                            # ★ ROBLOX 連携 ★
                            with gr.Accordion("🎮 ROBLOX連携 (Open Cloud API)", open=False, visible=False):
                                gr.Markdown("💡 AIをROBLOX内の仮想アバターと連動させるための設定です。[Creator Dashboard](https://create.roblox.com/credentials) から取得してください。")
                                roblox_api_key_input = gr.Textbox(
                                    label="ROBLOX API キー (Open Cloud)",
                                    type="password",
                                    placeholder="APIキーを入力",
                                    value="", # ui_handlersで部屋ごとの設定から読み込む
                                    interactive=True
                                )
            ...
            with gr.TabItem("アイテム") as item_root_tab:
                with gr.Tabs():
                    with gr.TabItem("食べ物"):
                        ...
                        gr.Markdown("---")
                        gr.Markdown("## 📦 インベントリ（所持品）")
                        food_inventory_df = gr.Dataframe(
                            headers=["ID", "アイテム名", "カテゴリ", "所持数", "作成者", "状態"],
                            interactive=True,
                            wrap=True
                        )
                        food_inventory_refresh_button = gr.Button("🔄 一覧を更新", variant="secondary")

                    with gr.TabItem("通常アイテム"):
                        ...
                        gr.Markdown("---")
                        gr.Markdown("## 📦 インベントリ（所持品）")
                        std_inventory_df = gr.Dataframe(
                            headers=["ID", "アイテム名", "カテゴリ", "所持数", "作成者", "状態"],
                            interactive=True,
                            wrap=True
                        )
                        std_inventory_refresh_button = gr.Button("🔄 一覧を更新", variant="secondary")
        ...
        demo.load(
            ...
            outputs=[food_inventory_df, std_inventory_df, food_use_item_dropdown]
        )
        ...
        place_item_button.click(
            ...
            outputs=[food_use_status, food_inventory_df, std_inventory_df, food_use_item_dropdown, location_item_dropdown, placed_at_furniture]
        ).then(
        ...
        pickup_item_button.click(
            ...
            outputs=[food_use_status, food_inventory_df, std_inventory_df, food_use_item_dropdown, location_item_dropdown]
        ).then(
            fn=lambda r, i: ui_handlers.handle_get_item_details(r, i, is_location=True),
            inputs=[current_room_name, location_item_dropdown],
            outputs=[location_item_details_markdown, location_item_image_preview]
        ...
        consume_location_item_button.click(
            ...
            inputs=[current_room_name, location_dropdown, location_item_dropdown, location_item_operation_amount],
            outputs=[food_use_status, location_item_dropdown, food_inventory_df, std_inventory_df, food_use_item_dropdown, chat_input_multimodal]
        ).then(
            fn=lambda r, i: ui_handlers.handle_get_item_details(r, i, is_location=True),
            inputs=[current_room_name, location_item_dropdown],
            outputs=[location_item_details_markdown, location_item_image_preview]
        ...
        copy_inventory_item_button.click(
            ...
            inputs=[current_room_name, food_use_item_dropdown],
            outputs=[food_use_status, food_inventory_df, std_inventory_df, food_use_item_dropdown]
        ).then(
            fn=lambda r, i: ui_handlers.handle_get_item_details(r, i, is_location=False),
            inputs=[current_room_name, food_use_item_dropdown],
            outputs=[item_details_markdown, food_use_item_image_preview]
        ...
        item_op_confirm_state.change(
            ...
            inputs=[item_op_confirm_state, food_use_item_dropdown, food_editor_selection_state, std_item_editor_selection_state],
            outputs=[food_use_status, food_inventory_df, std_inventory_df, food_use_item_dropdown]
        )

        # 4. 作成画面での編集・削除
        # 食べ物
        food_inventory_df.select(
            fn=ui_handlers.handle_row_selection,
            inputs=[food_inventory_df],
            outputs=[food_editor_selection_state]
        )
        ...
        # 通常アイテム
        std_inventory_df.select(
            fn=ui_handlers.handle_row_selection,
            inputs=[std_inventory_df],
            outputs=[std_item_editor_selection_state]
        )
        ...
        food_item_save_button.click(
            ...
            outputs=[food_item_status, food_inventory_df, std_inventory_df, food_use_item_dropdown]
        )

        food_inventory_refresh_button.click(
            fn=ui_handlers.handle_refresh_food_inventory,
            inputs=[current_room_name],
            outputs=[food_inventory_df, std_inventory_df, food_use_item_dropdown]
        )
        ...
        std_item_save_button.click(
            ...
            outputs=[std_item_status, food_inventory_df, std_inventory_df, food_use_item_dropdown]
        )

        std_inventory_refresh_button.click(
            fn=ui_handlers.handle_refresh_food_inventory,
            inputs=[current_room_name],
            outputs=[food_inventory_df, std_inventory_df, food_use_item_dropdown]
        )
        ...
        food_consume_button.click(
            ...
            inputs=[food_use_item_dropdown, current_room_name],
            outputs=[food_use_status, food_inventory_df, std_inventory_df, food_use_item_dropdown, food_use_item_image_preview, chat_input_multimodal]
        ).then(
            fn=ui_handlers.update_api_history_limit_state_and_reload_chat,
            inputs=[
                room_api_history_limit_dropdown, 
...
# --- Multi-Provider Events ---
        ...
        openai_profile_dropdown.change(
            ...
            outputs=[openai_base_url_input, openai_api_key_input, openai_model_dropdown, openai_temperature_slider, openai_top_p_slider, openai_max_tokens_input]
        )
        ...
        image_gen_provider_radio.change(
            ...
            outputs=[gemini_model_section, openai_image_section, image_gen_api_key_dropdown]
        )
        
        save_image_gen_button.click(
            fn=ui_handlers.handle_save_image_generation_settings,
            inputs=[image_gen_provider_radio, image_gen_api_key_dropdown, gemini_image_model_dropdown, openai_image_profile_dropdown, openai_image_model_dropdown],
            outputs=None
        )
