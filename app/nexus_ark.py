...
""")
                            ...
                            with gr.Accordion("🖼️ 画像生成 (ユーザー用)", open=False):
                                with gr.Row():
                                    ...
                                    user_gen_image_openai_profile = gr.Dropdown(
                                        choices=_openai_profiles,
                                        value=config_manager.CONFIG_GLOBAL.get("image_generation_openai_settings", {}).get("profile_name") if config_manager.CONFIG_GLOBAL.get("image_generation_openai_settings", {}).get("profile_name") in _openai_profiles else (_openai_profiles[0] if _openai_profiles else None),
                                        label="プロファイル",
                    ...
                    with gr.TabItem("📌 コンテキスト管理"):
                        ...
                        with gr.Accordion("🧠 ワーキングメモリ (動的コンテキスト)", open=False):
                            ...
                            with gr.Accordion("スロットメタデータ", open=False):
                                ...
                                with gr.Row():
                                    reload_working_memory_metadata_button = gr.Button("メタデータ再読込", variant="secondary")
            ...
            with gr.TabItem("知識") as knowledge_tab:
                ...
                with gr.Accordion("📚 知識ファイル", open=False):
                    ...
                    knowledge_status_output = gr.Textbox(
                        label="ステータス",
                        value="一覧は未読込です。現在の知識ファイルを確認するには「一覧を更新」を押してください。",
                        interactive=False,
                    )
                ...
                with gr.Tabs():
                    with gr.TabItem("記憶"):
                        ...
                        with gr.Accordion("🎯 Purpose Profile (目的意識)", open=False):
                            ...
                            with gr.Row():
                                init_purpose_profile_button = gr.Button("初期化/読込", variant="primary")
                                save_purpose_profile_button = gr.Button("保存", variant="secondary")
                                reload_purpose_profile_button = gr.Button("再読込", variant="secondary")
                            with gr.Row():
                                purpose_proposal_id_input = gr.Textbox(
                                    label="承認する proposal_id",
                                    interactive=True,
                                    scale=3
                                )
                        ...
                        with gr.Accordion("🔬 研究・分析ノート", open=False):
                            ...
                            with gr.Accordion("🧵 Research Threads（継続研究スレッド）", open=False):
                                with gr.Row():
                                    ...
                                    refresh_research_threads_button = gr.Button("一覧を更新", variant="primary", scale=1)
                                ...
                                with gr.Row():
                                    ...
                                    reload_research_thread_body_button = gr.Button("スレッド本文を再読込", variant="secondary")
        ...
        room_individual_settings_inputs = [
            current_room_name, room_tts_provider_dropdown, room_tts_model_dropdown, room_voice_dropdown, room_voice_style_prompt_textbox
        ] + gen_settings_inputs + [
            enable_typewriter_effect_checkbox,
            streaming_speed_slider,
        ] + [
        ...
        working_memory_slot_dropdown.change(
            ...
            outputs=[working_memory_editor, active_working_memory_status]
        )
        working_memory_new_slot_button.click(
            fn=ui_handlers.handle_new_working_memory_slot,
            inputs=[current_room_name],
            outputs=[working_memory_slot_dropdown, working_memory_editor, active_working_memory_status]
        )
        ...
        save_working_memory_metadata_button.click(
