...
""")
                        ...
                        with gr.TabItem("個別") as individual_settings_tab:
                            ...
                            with gr.Accordion("🗓️ カレンダー連携（このルーム）", open=False):
                                ...
                                room_gcal_write_dropdown = gr.Dropdown(
                                    ...
                                    info="ペルソナが自由に予定を書き込める専用カレンダー。未指定なら書き込みは無効です。",
        ...
        demo.load(
            ...
            inputs=[current_room_name],
            outputs=[room_gcal_inject_cb, room_gcal_reminder_cb, room_gcal_write_dropdown],
            show_progress="hidden"
        ).then(
        ...
        # 3. [v6] アバターモードラジオを更新
            ...
            # このルームのカレンダー個別設定を読み込む（is_switching_room=True のうちに行い
            # .change による誤保存を抑止する。巨大な統合出力リストは触らず自前の小さな出力で）
            fn=ui_handlers.load_room_calendar_settings,
            inputs=[room_dropdown],
            outputs=[room_gcal_inject_cb, room_gcal_reminder_cb, room_gcal_write_dropdown],
            show_progress="hidden"
        ).then(
            # このルームの自動レビュー設定を反映（巨大出力は触らず自前の小さな出力で）
            fn=ui_handlers.load_room_review_settings,
            inputs=[room_dropdown],
            outputs=[
                room_agent_delegation_review_iterations_number,
        ...
        room_autonomous_inactivity_preset.change(
            fn=lambda value: gr.update(value=value) if value is not None else gr.update(),
            inputs=[room_autonomous_inactivity_preset],
            outputs=[room_autonomous_inactivity_slider],
            show_progress="hidden",
            queue=False,
        )
