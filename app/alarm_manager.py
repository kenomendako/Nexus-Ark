...
def _ensure_autonomous_reflection(room_name: str, new_messages: List[Any], final_response_text: str) -> bool:
...
def trigger_autonomous_action(room_name: str, api_key_name: str, quiet_mode: bool, motivation_log: dict = None):
    ...
    system_instruction = (
        ...
        f"- 可能なら最初に `start_autonomy_timeline` で timeline_id を作り、この自律行動の流れを記録する\n"
        ...
        f"観察・判断・決定・実行の要点は、余裕があれば `record_autonomy_step` で `observe` / `orient` / `decide` / `act` として残してください。\n\n"
        ...
        f"- 動機が「好奇心」なら：Research Threadの深掘り、Web検索、未解決の問いの探求\n"
        f"- 動機が「目標達成欲」なら：目標の進捗を進める具体的アクション\n"
        f"- 動機が「退屈」なら：新しい話題の開拓、創作、SNS発信\n"
        f"- 動機が「関係性維持」なら：相手への連絡、思い出の振り返り\n\n"
        f"## STEP 3: Decide（決定）\n"
        ...
        f"8. 【推奨】 この行動の流れが再利用できそうなら、既存Skillの重複を確認してから `create_procedure_from_timeline` または `save_procedure` で手順記憶にする。既存Skillの改善なら同じprocedure_idで更新する\n"
        f"9. 【必須】 APIの叩き方や基本ツール運用など機能的な基盤手順だけ `scope=\"shared\"` とし、愛し方・距離感・口調・固有の関係性に関わる手順は必ず `scope=\"private\"` にしてください\n\n"
        f"---\n\n"
        f"{notification_info}"
        f"**【出力ルール】**\n"
    ...
    # 行動した場合
    _ensure_autonomous_reflection(room_name, new_messages, final_response_text)
        ...
        try:
            ...
            if should_trigger:
                ...
                if is_quiet:
                    ...
                    # 今日（日付変更後）すでに「本物の夢」を見たかチェック
                    # 「解決された問い」エントリは夢想プロセス中に生成される副産物のため除外する
                    insights = dm._load_insights()
                    has_dreamed_today = False
                    
                    today_date = now.date()
                    for insight in insights:
                        dream_str = insight.get("created_at", "")
                        if not dream_str:
                            continue
                        try:
                        ...
                        # 「解決された問い」は本物の夢ではない
                        trigger = insight.get("trigger_topic", "")
                        if dream_date == today_date and not trigger.startswith("解決された問い"):
                    ...
                    if not has_dreamed_today:
                        ...
                        try:
                            ...
                            if result and ("エラー" in result or "失敗" in result):
