...
def trigger_alarm(alarm_config, current_api_key_name):
    ...
    if response_text and not response_text.startswith("[エラー"):
        ...
        existing_timestamp_match = re.search(timestamp_pattern, raw_response)
        if existing_timestamp_match:
            print(f"--- [AI模倣タイムスタンプ除去] ---")
            raw_response = re.sub(timestamp_pattern, '', raw_response)
...
def trigger_autonomous_action(room_name: str, api_key_name: str, quiet_mode: bool, motivation_log: dict = None):
    ...
    # 英語曜日（Sun等）と日本語曜日（日）の両形式に加え、モデル名（| gemini...）まで対応
    timestamp_pattern = r'\n\n\d{4}-\d{2}-\d{2}\s*\([A-Za-z月火水木金土日]{1,3}\)\s*\d{2}:\d{2}:\d{2}(?:\s*\|\s*.*)?'
    existing_timestamp_match = re.search(timestamp_pattern, final_response_text)
    if existing_timestamp_match:
        print(f"--- [AI模倣タイムスタンプ除去] ---")
        final_response_text = re.sub(timestamp_pattern, '', final_response_text)
