...
import contextlib
import glob
from file_lock_utils import locked_file, safe_append_text

LOCK_FILE_PATH = Path("nexus_ark.lock")
_MIGRATION_DONE_CACHE = set()

def _current_month_log_file_for_room_dir(room_dir: str) -> str:
    current_month = datetime.datetime.now().strftime("%Y-%m")
    return os.path.join(room_dir, constants.LOGS_DIR_NAME, f"{current_month}.txt")

def _resolve_monthly_log_file(log_file_path: str) -> str:
    ...
    return _current_month_log_file_for_room_dir(room_dir)

def normalized_text_key(text: str) -> str:
    """Return a compact NFKC/casefold key for short-text dedup checks."""
...
def normalized_text_similarity(left: str, right: str) -> float:
    """Return SequenceMatcher similarity after project-standard text normalization."""
    normalized_left = normalized_text_key(left)
    normalized_right = normalized_text_key(right)
...
def invalidate_log_migration_cache(room_dir: Optional[str] = None):
    """
    ログ移行済み判定のプロセス内キャッシュを無効化する。
    オンボーディング移行のように同じ room_dir をフォルダごと差し替える処理では、
    以前の判定結果を使い回すと新しくコピーされた log.txt の移行がスキップされる。
    """
    if room_dir:
        variants = {
            room_dir,
            ...
            os.path.abspath(room_dir).replace("\\", "/"),
        }
        for variant in variants:
            _MIGRATION_DONE_CACHE.discard(variant)
    else:
        _MIGRATION_DONE_CACHE.clear()
...
def remove_ai_timestamp(text: str) -> str:
    ...
    # 末尾の空白・改行（\s*$）も許容する：タイムスタンプの後ろに改行や空白が残ると
    # 以前は $ にマッチせず除去漏れになっていた（例: "...10:59:30\n\n"）。
    timestamp_pattern = r'(?:\n\n)?\d{4}-\d{2}-\d{2}\s*\([A-Za-z月火水木金土日]{1,3}\)\s*\d{2}:\d{2}:\d{2}(?:\s*\|\s*.*)?\s*$'
...
def acquire_lock() -> bool:
    ...
    try:
        ...
        if pid == os.getpid():
            print("\n" + "!"*60)
            print("警告: ロックファイルのPIDが現在の起動プロセスと一致しています。")
            print("  前回の異常終了後にPIDが再利用された可能性があるため、ロックを作り直します。")
            print("!"*60 + "\n")
        ...
        if pid and psutil and psutil.pid_exists(pid):
            ...
            try:
                ...
                is_same_app_process = (
                    "nexus_ark.py" in cmdline
                    and (not lock_path or os.path.abspath(proc_cwd) == os.path.abspath(lock_path))
                )
            except Exception:
                is_same_app_process = True

            if not is_same_app_process:
                print("\n" + "!"*60)
                print("警告: ロックファイルのPIDは存在しますが、Nexus Arkのプロセスではありません。")
                print(f"  - 記録されていたPID: {pid}")
                print("  PID再利用と判断し、古いロックファイルを削除して処理を続行します。")
                print("!"*60 + "\n")
                LOCK_FILE_PATH.unlink()
                time.sleep(0.5)
                _create_lock_file()
...
_file_message_count_cache: Dict[str, Tuple[float, int]] = {}
...
def invalidate_chat_log_cache(file_path: str):
    ...
    # パス表記の揺れ（区切り文字、絶対パス化など）を吸収して確実にキャッシュを消去する
    paths_to_check = [
        file_path,
        file_path.replace("\\", "/"),
        file_path.replace("/", "\\"),
    ...
    for p in paths_to_check:
        if p in _file_log_cache:
            del _file_log_cache[p]
        if p in _file_message_count_cache:
            del _file_message_count_cache[p]


def _chat_log_clear_marker(room_dir: str, month: str) -> str:
    return os.path.join(room_dir, constants.LOGS_DIR_NAME, f".cleared-{month}")


def mark_chat_log_month_cleared(log_file_path: str) -> None:
    """明示的に空保存した月を記録し、「最新」の過去月フォールバックを防ぐ。"""
    room_dir = _get_room_dir_from_path(log_file_path)
    month = os.path.splitext(os.path.basename(log_file_path))[0]
    if not re.match(r"^\d{4}-\d{2}$", month):
        return
    logs_dir = os.path.join(room_dir, constants.LOGS_DIR_NAME)
    os.makedirs(logs_dir, exist_ok=True)
    with open(_chat_log_clear_marker(room_dir, month), "w", encoding="utf-8") as f:
        f.write(f"cleared_at={datetime.datetime.now().isoformat()}\n")


def clear_chat_log_month_marker(log_file_path: str) -> None:
    """新しい発言または非空編集が保存された月の削除マーカーを解除する。"""
    room_dir = _get_room_dir_from_path(log_file_path)
    month = os.path.splitext(os.path.basename(log_file_path))[0]
    marker = _chat_log_clear_marker(room_dir, month)
    if os.path.exists(marker):
        os.remove(marker)


def is_chat_log_month_cleared(log_file_path: str) -> bool:
    room_dir = _get_room_dir_from_path(log_file_path)
    month = os.path.splitext(os.path.basename(log_file_path))[0]
    return os.path.exists(_chat_log_clear_marker(room_dir, month))

# ... (backup_and_repair_json は省略) ...


def _count_log_messages_in_file(file_path: str) -> int:
    """本文を保持せず、ログファイル内のメッセージヘッダー数だけを数える。"""
    try:
        mtime = os.path.getmtime(file_path)
    except OSError:
        return 0

    cached = _file_message_count_cache.get(file_path)
    ...
    try:
        ...
        print(f"エラー: ログファイル '{file_path}' 件数カウントエラー: {e}")
        return 0

    _file_message_count_cache[file_path] = (mtime, count)
    return count
...
def load_chat_log_lazy(
    ...
    for f_rem in remaining_files:
        offset_from_skipped_files += _count_log_messages_in_file(f_rem)
...
def _migrate_chat_logs(room_dir: str):
    ...
    if room_dir in _MIGRATION_DONE_CACHE:
        cached_logs_dir = os.path.join(room_dir, constants.LOGS_DIR_NAME)
        cached_legacy_log = os.path.join(room_dir, "log.txt")
        cached_has_monthly = bool(glob.glob(os.path.join(cached_logs_dir, "20??-??.txt")))
        cached_has_legacy = os.path.exists(cached_legacy_log)
        if cached_has_monthly or not cached_has_legacy:
            return
        print(f"--- [Log Migration] キャッシュ済みですがlegacy logを検知したため再評価します: {room_dir} ---")
    ...
    if os.path.exists(marker_file):
        # マーカーがあっても、肝心のlogsフォルダ内が空で、かつ移行対象がある場合は再試行を許可する
        has_monthly_logs = any(f.endswith(".txt") for f in os.listdir(new_logs_dir)) if os.path.exists(new_logs_dir) else False
        if has_monthly_logs:
    ...
    # 【追加】以前の移行が中断された形跡 (.migrating) があれば復旧を試みる
    for tmp_f in glob.glob(os.path.join(room_dir, "*.migrating")):
        try:
            legacy_orig = tmp_f.replace(".migrating", "")
            if not os.path.exists(legacy_orig):
                os.rename(tmp_f, legacy_orig)
        except Exception: pass
    if os.path.exists(legacy_archives_dir):
        for tmp_f in glob.glob(os.path.join(legacy_archives_dir, "*.migrating")):
            try:
                legacy_orig = tmp_f.replace(".migrating", "")
                if not os.path.exists(legacy_orig):
                    os.rename(tmp_f, legacy_orig)
    ...
    for f_path in migration_targets:
        ...
        try:
            ...
            fallback_month = datetime.datetime.fromtimestamp(os.path.getmtime(f_path)).strftime("%Y-%m")
            current_month = fallback_month # 日付不明用
            ...
            if not messages_by_month:
                fallback_month = datetime.datetime.fromtimestamp(os.path.getmtime(f_path)).strftime("%Y-%m")
                messages_by_month[fallback_month] = [raw_content.rstrip() + "\n\n"]
...
def save_message_to_log(log_file_path: str, header: str, text_content: str) -> Optional[str]:
    ...
    try:
        ...
        target_log_file = _resolve_monthly_log_file(log_file_path)
        clear_chat_log_month_marker(target_log_file)
        content_to_append = f"{header.strip()}\n{text_content.strip()}\n\n"
        with locked_file(target_log_file):
            ...
            safe_append_text(target_log_file, content_to_append)
        
        # ログ書き込み後にキャッシュを無効化
        # 引数として渡されたパスと、実際に書き込んだパスの両方を無効化する
        invalidate_chat_log_cache(log_file_path)
        invalidate_chat_log_cache(target_log_file)
...
def delete_message_from_log(log_file_path: str, message_to_delete: Dict[str, str], abs_index: Optional[int] = None) -> Optional[str]:
    """
        ...
        for f_path in log_files:
            try:
                with locked_file(f_path):
                    ...
                    for i, match in enumerate(matches):
                        ...
                        if message_content == target_content and responder == target_responder:
                            if abs_index is not None:
                                if current_base_index + i == abs_index:
                                    found_index = i
                                    break
                            else:
                                found_index = i
                    ...
                    current_base_index += len(matches)
...
_LINE_PAIRED_THOUGHT_RE = re.compile(
    r"^[ \t]*\[\s*THOUGHTS?\s*\][ \t]*\r?\n[\s\S]*?\r?\n[ \t]*\[/\s*THOUGHTS?\s*\][ \t]*$"
    r"|^[ \t]*【\s*Thoughts?\s*】[ \t]*\r?\n[\s\S]*?\r?\n[ \t]*【/\s*Thoughts?\s*】[ \t]*$"
    r"|^[ \t]*<thinking>[ \t]*\r?\n[\s\S]*?\r?\n[ \t]*</thinking>[ \t]*$",
...
_LINE_PAIRED_THOUGHT_EXTRACT_RE = re.compile(
    r"(^[ \t]*\[\s*THOUGHTS?\s*\][ \t]*\r?\n([\s\S]*?)\r?\n[ \t]*\[/\s*THOUGHTS?\s*\][ \t]*$"
    r"|^[ \t]*【\s*Thoughts?\s*】[ \t]*\r?\n([\s\S]*?)\r?\n[ \t]*【/\s*Thoughts?\s*】[ \t]*$"
    r"|^[ \t]*<thinking>[ \t]*\r?\n([\s\S]*?)\r?\n[ \t]*</thinking>[ \t]*$)",
    re.IGNORECASE | re.MULTILINE,
...
def strip_orphan_thought_tags(text: str) -> str:
    """
    ペア抽出後に本文へ残った思考境界タグだけを取り除く。
    Gradio 6 metadata表示ではタグ自体を本文に見せない。
    """
    if not text:
        return ""
    return _THOUGHT_BOUNDARY_TAG_RE.sub("", text).strip()
...
def remove_thoughts_from_text(text: str) -> str:
    ...
    # Gradio 6ではタグを行単独で出すため、まず行境界にある本物のブロックを除去する。
    # 思考本文中に説明として現れる "[THOUGHT]...[/THOUGHT]" は閉じタグ扱いしない。
    text = _LINE_PAIRED_THOUGHT_RE.sub("", text)

    # 後方互換: 古いログに残る同一行の短いタグブロックも除去する。
    ...
    text = strip_orphan_thought_tags(text)
...
def extract_thoughts_from_text(text: str) -> str:
    ...
    consumed_ranges = []
    for match in _LINE_PAIRED_THOUGHT_EXTRACT_RE.finditer(text):
        thought = match.group(2) or match.group(3) or match.group(4)
        if thought and thought.strip():
            thoughts.append(thought.strip())
            consumed_ranges.append((match.start(), match.end()))

    def _outside_line_blocks(index: int) -> bool:
        return not any(start <= index < end for start, end in consumed_ranges)

    # 後方互換: 同一行の古い短いタグブロックも拾う。
    patterns = [
    ...
    for pat in patterns:
        for match in re.finditer(pat, text, flags=re.IGNORECASE):
            if not _outside_line_blocks(match.start()):
                continue
            m = match.group(1)
            if m.strip():
...
def format_tool_result_for_ui(tool_name: str, tool_result: str) -> Optional[str]:
    ...
    # ユーザーへの表示が不要なシステム的ツール
    if tool_name in ["request_capability"]:
        return None
    ...
    # ファイル読み取り系には特別なエラー検知ロジックを適用
    # ファイル本文に "Exception:" や "に失敗しました" などが含まれることが頻繁にあるため、
    # ツール自体のエラーメッセージ（先頭の Error/【エラー】）のみを検出する。
    is_developer_tool = tool_name in ["list_project_files", "read_project_file"]
    is_read_only_content_tool = tool_name in {
        "read_research_notes",
        "read_creative_notes",
        "read_full_notepad",
        "read_diary_memory",
    ...
    if is_developer_tool or is_read_only_content_tool:
        # 読み取り系ツールの標準エラーフォーマットのみ検出
        if re.search(r"^(Error:|【エラー】|エラー:)", tool_result, re.IGNORECASE | re.MULTILINE):
    ...
    if tool_name == 'set_current_location':
    ...
    elif tool_name == 'send_user_notification':
        sent_match = re.search(r"通知を送信しました:\s*(.*)", tool_result, re.DOTALL)
        failed_match = re.search(r"通知の送信に失敗しました:\s*(.*)", tool_result, re.DOTALL)
        quiet_match = re.search(r"現在は通知禁止時間帯.*", tool_result, re.DOTALL)
        if sent_match:
            notification_text = sent_match.group(1).strip()
            request_match = re.search(r"\n?Pushover request:\s*([0-9a-fA-F-]+)\s*$", notification_text)
            request_text = ""
            if request_match:
                request_text = f"\n\nPushover request: `{request_match.group(1)}`"
                notification_text = notification_text[:request_match.start()].rstrip()
            display_text = f"スマホ通知を送信しました。\n\n{notification_text}{request_text}"
        elif failed_match:
            display_text = f"スマホ通知の送信に失敗しました。\n\n{failed_match.group(1).strip()}"
        elif quiet_match:
            display_text = quiet_match.group(0).strip()
        ...
        display_text = tool_result.strip()
    # 記憶検索ツール用のカスタムアナウンス
...
def is_tool_result_error(tool_name: str, tool_result: str) -> bool:
    """ツール結果がユーザーに見せるべき失敗かどうかを判定する。"""
    if not tool_result:
        return False
    ...
    return any(re.search(pattern, tool_result, re.IGNORECASE | re.MULTILINE) for pattern in generic_error_patterns)


def should_show_autonomous_tool_result(tool_name: str, tool_result: str) -> bool:
    """自律行動中のツール結果をチャット欄へ個別表示するかを返す。"""
    if not tool_name:
        return False
    if is_tool_result_error(tool_name, tool_result):
        return True
    if tool_name in constants.AUTONOMOUS_INTERNAL_TOOL_RESULTS:
        return False
    return tool_name in constants.AUTONOMOUS_VISIBLE_TOOL_RESULTS


def format_autonomous_action_summary(tool_messages: List[Any]) -> str:
    """チャット欄向けに、自律行動の内部ツール実行を短くまとめる。"""
    hidden_counts: Dict[str, int] = {}
    hidden_total = 0
    for msg in tool_messages:
        tool_name = getattr(msg, "name", "") or ""
        tool_result = str(getattr(msg, "content", "") or "")
        if should_show_autonomous_tool_result(tool_name, tool_result):
            continue
        hidden_counts[tool_name or "unknown_tool"] = hidden_counts.get(tool_name or "unknown_tool", 0) + 1
        hidden_total += 1
    ...
    parts = []
    for tool_name, count in sorted(hidden_counts.items()):
        parts.append(f"{tool_name} x{count}" if count > 1 else tool_name)
    tool_summary = " / ".join(parts[:8])
    if len(parts) > 8:
        tool_summary += f" / 他{len(parts) - 8}種"

    return (
        "🕯️ 自律行動の内部処理を記録しました。\n"
        f"- 実行: {tool_summary}\n"
        "- 詳細: ⚙️ Action Memory (直近のツール行動ログ) を確認してください。"
    )
...
def delete_and_get_previous_user_input(log_file_path: str, ai_message_to_delete: Dict[str, str], target_abs_index: Optional[int] = None) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        current_log_file = _resolve_monthly_log_file(log_file_path)
        with locked_file(current_log_file):
...
def truncate_chat_logs(room_dir: str, target_index: int):
    ...
    月をまたいだ暗黙削除は今月分ログ消失の原因になるため、ここでは対象ファイルだけを変更する。
    """
    logs_dir = os.path.join(room_dir, constants.LOGS_DIR_NAME)
    ...
    for f_path in log_files:
        try:
            with locked_file(f_path):
                ...
                if current_msg_count + file_msg_count > target_index:
                    ...
                    if 0 <= truncate_at_msg_idx < len(matches):
                        ...
                        new_content = content[:cut_pos].strip()
                        if new_content:
                            new_content += "\n\n"
            ...
            continue
    print(f"警告: ログ切り詰め対象インデックスが見つかりませんでした: {target_index}")
    ...
    if isinstance(content, list):
        ...
        for part in content:
            if isinstance(part, dict):
                ...
                if p_type == 'text' or part.get("thought") is True:
...
def extract_text_from_llm_content(raw_content) -> str:
    ...
    """
    text = ""
    if raw_content is not None and not isinstance(raw_content, (str, list, dict)) and hasattr(raw_content, "content"):
        return extract_text_from_llm_content(raw_content.content)
...
def append_system_message_to_log(room_name: str, message: str):
    ...
    try:
        ...
        if log_path:
            ...
            save_message_to_log(log_path, "## SYSTEM:System", f"{ts} | System\n{message}")
    except Exception as e:
