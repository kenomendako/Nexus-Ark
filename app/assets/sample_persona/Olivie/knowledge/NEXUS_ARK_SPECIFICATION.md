...
> **プロジェクトビジョン:** Nexus Arkの設計思想の詳細は [docs/VISION.md](VISION.md) を参照してください。
...
- **左カラム (Sidebar):** 
    - **ルームセレクター:** 現在対話するルーム（ペルソナ）を選択します。
    - **メニュー群:** 「⚙️ 設定」「⏰ 時間管理」「🧑‍🤝‍🧑 グループ会話」「🗨️ チャットルーム管理」「Update」など、アプリ全体の機能にアクセスします。
- **中央カラム (Main):** 
    - **チャットタブ:** AIとの対話を行うメインインターフェースです。
    - **コンテキスト管理タブ (📌):** ワーキングメモリ、添付ファイル、共有メモ帳（Notepad）、AIアクション履歴が集約されています。
    - **インベントリタブ (🎒):** ユーザーとペルソナの所持アイテムを管理・編集します。
    - **ログ・知識管理:** 過去の会話履歴の閲覧や、RAG用の知識ドキュメントを管理します。
- **右カラム (Profile/Location):** 
    - **プロフィール:** AIの名前、解説、アバター画像を表示します。
    - **情景映像/画像:** 情景描写システムによって生成・同期された現在の景色が表示されます。
    - **お出かけ・現在地:** ペルソナと「お出かけ」したり、ワールド内の現在地（仮想・一時的）を変更したりできます。
    - **表情管理:** 感情に応じたアバターの表情差分を登録します。
...
### 3.7. 発言の音声再生
AIの発言を選択し、「🔊 選択した発言を再生」ボタンを押すと、そのルームの「個別設定」で設定された声でテキストが読み上げられ、生成された音声ファイルは`audio_cache/`フォルダに保存されます。TTSプロバイダは Gemini / OpenAI公式 / OpenAI互換プロファイル / ElevenLabs に加え、ローカルの AivisSpeech / VOICEVOX / COEIROINK などのVOICEVOX互換HTTPエンジンを選択できます。
...
#### 5.1.1. AIプロバイダとモデル
Nexus Arkは多数のAIプロバイダに対応しており、各タスクごとに最適なモデルを選択できます。

- **Google Gemini:** ネイティブ対応のプロバイダ。Gemini 3.1 Pro / 3 Flash / 3.1Flash-Lite / 2.5 Pro などのモデルが利用可能です。
- **Anthropic (Claude):** Claudeの各モデルに対応。
- **OpenAI互換:** OpenAI API互換のプロファイル（OpenRouter、Groq、Moonshot AI、Zhipu AI、Ollama、X.ai、NVIDIA NIM など）を無制限に登録・管理できます。
- **無料プロバイダ:** 
    - **Pollinations.ai:** テキスト生成および画像生成（Flux等）が無料で利用可能です。
    - **Hugging Face:** Inference API を介して、Llama 3 や Flux.1-schnell などのオープンモデルを利用可能です。
- **ローカルLLM:** 
    - **Ollama:** ローカルで動作する Ollama サーバーと連携。
    - **GGUF直接ロード:** `llama-cpp-python` を使用し、PC内の GGUF ファイルを直接ロードして実行（GPU加速対応）。

#### 5.1.2. 内部タスクモデルの個別設定
要約や検索クエリ生成、思考ログの翻訳などの「裏方」のタスクに使うモデルを個別に指定できます。
- **要約 / 情景描写:** 会話の要約や場所の描写文生成に使用。
- **翻訳:** 思考ログの翻訳や、外部連携時のテキスト変換に使用。
- **RAG / エンベディング:** 記憶のベクトル化と検索。ローカルエンベディングモデル（`multilingual-e5-small` 等）も使用可能です。

#### 5.1.3. APIキー / Webhook管理 UI
🔑 アイコンの管理画面が刷新され、プロバイダごとに整理されました。
- **共通キー設定:** Gemini, OpenAI公式, Anthropic 等の主要キーを一括管理。
- **プロファイル管理:** 各種 OpenAI 互換サービスの接続情報を名前をつけて保存（プロファイル化）。
- **外部キー:** Pollinations, Hugging Face, Tavily 等のキー設定。

#### 5.1.4. 画像生成設定
- **プロバイダ選択:** Gemini, Pollinations.ai, Hugging Face, OpenAI互換から選択。
- **モデル指定:** 各プロバイダが提供する最新の画像生成モデル（Flux.1, DALL-E 3 等）を指定可能。
- **EXIF修正:** スマホ写真などの回転メタデータを自動修正する機能。

#### 5.1.5. 検索プロバイダ設定
「🔍 検索プロバイダ設定」から、AIがWeb検索を行う際に使用するサービスを選択できます。
- **Gemini Grounding:** Gemini APIのグラウンディング機能を使用（デフォルト）
- **Tavily:** Tavily APIを使用（要APIキー設定）
- **DuckDuckGo:** 高速・無料の検索エンジン
...
- **通知サービス:** アラーム/タイマー用と、AIツールや承認要請などの通常通知用に、それぞれ「Discord」と「Pushover」のどちらを使用するか選択します。
...
- **音声設定:** 発言の音声再生に使用するTTSプロバイダ、モデルまたはローカルエンジンURL、声の種類（`tts_voice` / 旧`voice_id`）と、声のトーンを指示するスタイルプロンプト（`voice_style_prompt`）を設定します。AivisSpeech / VOICEVOX / COEIROINK ではAPIキー不要で、各エンジンをローカル起動して利用します。
...
- **ルームごとのテーマ:** 各ルームの個別設定で、ルーム専用のCSSテーマを設定できます。現在地画像を背景として設定することも出来ます。設定を適用するにはアプリケーションの再起動が必要です。
...
- **通知の仕組み:** 設定時刻になると、指定されたルームのAIが内容に基づいた応答を生成し、アラーム用に設定された通知サービス（Discord/Pushover）およびPCのデスクトップ通知でメッセージを送信します。
...
#### 7.5.5. 外部活動記憶 (External Codex)
Twitter (X) や外部サービスでの活動を記録するシステムです。
- **活動ログ:** AIによるツイート、リプライ、通知確認の履歴を構造化して保存します。
- **コンテキスト注入:** 直近の活動サマリーがシステムプロンプトの `{twitter_activity_section}` へ自動注入されます。
- **エンティティ連携:** 外部で交流した相手を「エンティティ記憶」の候補として自動提案します。

#### 7.5.6. 会話ログが長くなった時の自動要約
...
**インベントリ・アイテム:**
- `place_item_to_location`: アイテムを場所に置く
- `pickup_item_from_location`: 場所からアイテムを拾う
- `examine_item`: アイテムを鑑定する
- `gift_item_to_user`: ユーザーにアイテムを贈る
- `consume_item_from_location`: その場にある食べ物を消費する
...
*この仕様書はNexus Ark v1.0 (Beta)（2026-04）の機能を反映しています。*

---

## 16. 統合インベントリ管理

Nexus Arkでは、ユーザーとペルソナが所持するアイテムを一括管理する「インベントリ」システムが導入されています。

### 16.1. インベントリタブ (🎒)
中央カラムのインベントリタブから、以下の情報を確認・操作できます。
- **ユーザーの所持品:** ユーザーが現在持っているアイテム。
- **ペルソナの所持品:** AIペルソナが現在持っているアイテム。
- **この場所にある物:** 現在の「場所（Location）」に配置されているアイテム。
- **アイテム詳細:** 選択したアイテムの画像、名前、フレーバーテキスト。

### 16.2. アイテムの種類
- **通常アイテム:** 道具、記念品、武器など、多様な種類のアイテム。
- **食べ物アイテム:** 「消費可能」なアイテム。味、香り、テクスチャ、共感覚イメージなどの詳細な五感情報を持っています。

### 16.3. 主な操作
- **作成:** ユーザーまたはAIが、名前、画像、説明、属性を指定して新しいアイテムを生成できます。AIが作成する場合、フレーバーテキストに基づいた画像を自動生成することも可能です。
- **贈与:** ユーザーからAIへ、またはAIからユーザーへアイテムを渡すことができます。
- **配置/拾得:** インベントリのアイテムを現在の場所に「置く」、または場所にあるアイテムをインベントリに「拾う」ことができます。
- **消費/味わう:** 食べ物アイテムを消費します。AIが消費した場合、その五感体験が内的状態（感情やArousal）に影響を与え、ログに記録されます。
- **鑑定 (Examine):** アイテムを詳細に観察し、隠された情報を引き出したり、フレーバーテキストを読み解いたりします。

### 16.4. AIツール
AIは以下のツールを用いて、状況に応じた自律的なアイテム操作を行います。
- `place_item_to_location`: アイテムを場所に置く
- `pickup_item_from_location`: 場所からアイテムを拾う
- `examine_item`: アイテムを鑑定する
- `gift_item_to_user`: ユーザーにアイテムを贈る
