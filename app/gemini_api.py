...
def invoke_nexus_agent_stream(agent_args: dict) -> Iterator[Dict[str, Any]]:
    ...
    history_loaded_images = []
    
    for msg in reversed(messages[-15:]): # 最新15メッセージを後ろからスキャン
        if isinstance(msg.content, str):
            found_images = view_image_pattern.findall(msg.content)
            for img_path in reversed(found_images): # 見つかった順（＝新しい順）に処理
                if img_path not in history_loaded_images and os.path.exists(img_path):
                    history_loaded_images.append(img_path)
    ...
    if history_loaded_images:
        for img_path in history_loaded_images:
            try:
                ...
                if kind and kind.mime.startswith('image/'):
                    ...
                    if resize_result:
                        ...
                        mime_type = kind.mime
                    final_prompt_parts.append({"type": "text", "text": f"- [(Memory Load) {display_name}]"})
...
{thought_text}"""
        ...
        provider, model_name_raw = get_effective_internal_model("translation")
