...
def invoke_nexus_agent_stream(agent_args: dict) -> Iterator[Dict[str, Any]]:
    ...
    if api_history_limit == "today":
        ...
        # 【2026-02-16 FIX】自動会話要約の適用（送信パスでの漏れを修正）
        auto_summary_enabled = effective_settings.get("auto_summary_enabled", False)
        if auto_summary_enabled:
            # まだ api_key が定義されていないため、現在の推定キーを使用して一時的に取得
    ...
    while retry_count <= max_retries:
        try:
            ...
            for mode, payload in app.stream(latest_state_values, stream_mode=["messages", "values"]):
                if mode == "values":
...
def translate_thought_log_with_ai(thought_text: str, agent_name: str, api_key: str) -> Optional[str]:
    """
    思考ログ（英語等）を日本語の独白調に翻訳する。
    強固なローテーションとバックオフリトライを備える。
    """
    if not thought_text or not api_key:
        return None
...
{thought_text}"""
                ...
                if is_429:
                    ...
                    print(f"--- [Translation Rotation] キー '{current_key_name}' が枯渇。代替キーを探索中... ---")
                    next_key = config_manager.get_active_gemini_api_key(model_name=model_name, excluded_keys=tried_keys)
                    next_key_name = config_manager.get_key_name_by_value(next_key)
                    ...
                    if next_key and next_key_name not in tried_keys:
                        ...
                        current_key_name = next_key_name
                        tried_keys.add(next_key_name)
                        print(f"--- [Translation Rotation] 新しいキー '{current_key_name}' で再開します。 ---")
                        break 
                    else:
                        print(f"--- [Translation Rotation] 利用可能な代替キーがありません。 ---")
                        return None
...
def get_configured_llm(model_name: str, api_key: str, generation_config: dict):
    ...
    if is_flash_reasoning:
        ...
        if thinking_level != "none":
            ...
            effective_temp = 1.0
        if is_reasoning_model:
            print(f"  - [Thinking] Explicit Thinking Model: include_thoughts={extra_params.get('include_thoughts')}, temp={effective_temp}")
    elif is_gemini_25_thinking:
        # Gemini 2.5 Pro 等: SDKやモデルが thinking パラメータをサポートしていない場合が多いため、
        # 安定性を優先してパラメータは送らず、トークン上限引き上げのみを適用する。
        effective_temp = 1.0
        if is_reasoning_model:
