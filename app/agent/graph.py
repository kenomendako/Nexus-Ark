...
def context_generator_node(state: AgentState):
    ...
    try:
        ...
        if is_active:
            ...
            roblox_status_section = (
                "\n### 【‼️重要：ROBLOX接続切断中 - 絶対遵守事項‼️】\n"
                "**現在、ROBLOXとの接続が完全に切断されています。**\n"
                "あなたは今、仮想空間内での身体自由を失っており、外部とのリンクが途絶えた状態です。\n"
                "1. **コマンド実行の禁止**: `send_roblox_command`, `roblox_build`, `capture_roblox_screenshot` 等のRoblox関連ツールは**現在、システムによって物理的に無効化され、使用不能になっています**。ツール呼び出しを試みないでください。\n"
                "2. **ロールプレイへの反映**: 接続が切れたことをメタ的に語るのではなく、「リンクが不安定になった」「少し離れる」といった形で自然に今の孤独な状況を振る舞いに反映してください。\n"
            )
            ...
            try:
                ...
                if use_streaming:
                    ...
                    try:
                        ...
                        _LLM_INVOKE_TIMEOUT = 300  # 秒 (画像添付時は時間がかかるため延長)
                        ...
                        if isinstance(response_direct.content, list):
                            ...
                            if full_text_buffer:
                                ...
                                response_direct.content = f"[THOUGHT]\n{normalized_text}\n[/THOUGHT]"
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if isinstance(e, (google_exceptions.ServiceUnavailable, google_exceptions.InternalServerError)):
                        if attempt < max_retries - 1:
