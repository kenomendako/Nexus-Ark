...
def context_generator_node(state: AgentState):
    ...
    try:
        ...
        if twitter_settings.get("enabled", False):
    ...
    if not is_active and roblox_filtering_enabled:
        ...
        # Twitterツールフィルタリング (必要に応じて追加。現状はRegistry側で制御)
        # twitter_tools = ["draft_tweet", "post_tweet"]
        # if not _is_twitter_active(room_name): ...
            ...
            try:
                ...
                if not chunks or (not has_tools and not content_str.strip()):
                    if attempt < max_agent_retries:
                        print(f"  - [Retry] AIからの有効な応答が得られませんでした。再試行します... ({attempt+1}/{max_agent_retries})")
                        time.sleep(2)
                        continue # 次の試行へ
