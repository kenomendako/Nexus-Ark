...
def agent_node(state: AgentState):
    ...
    is_gemini_3_flash = _is_gemini_3_flash_family(state.get('model_name', ''))
    ...
    if autonomy_finalization_pending:
        ...
        try:
            ...
            requested_category = registry.extract_recent_requested_capability(state.get("messages", []))
            if requested_category:
                image_generation_enabled = config_manager.CONFIG_GLOBAL.get("image_generation_mode", "new") != "disabled"
                current_tools = registry.get_tools_for_capability(
                    room_name=room_name,
    ...
    # --- ファイル編集系（プランニング＆反映） ---
        ...
        try:
            ...
            if mod_req is None or str(mod_req).strip() == "" or str(mod_req).strip() == "None":
                raise ValueError("エラー: 編集内容(modification_request)が指定されていないか、無効な値('None'など)です。引数を再確認してください。")
...
def safe_tool_executor(state: AgentState):
    ...
    if loop_count >= MAX_TOOL_LOOPS:
        ...
        print(f"  - [Safety] ループ上限({MAX_TOOL_LOOPS})に達したため、次ターンでは後始末ツール以外を禁止し最終応答を促します。")
