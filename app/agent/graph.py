...
def agent_node(state: AgentState):
    ...
    if state.get('tool_use_enabled', True):
        try:
            ...
            requested_capability_text = ""
            if state.get("messages") and isinstance(state["messages"][-1], ToolMessage):
                last_tool_msg = state["messages"][-1]
                if getattr(last_tool_msg, "name", "") == "request_capability":
                    requested_capability_text = last_tool_msg.content if isinstance(last_tool_msg.content, str) else str(last_tool_msg.content)

            requested_category = registry.extract_requested_capability(requested_capability_text)
