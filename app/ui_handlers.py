...
def _update_chat_tab_for_room_change(room_name: str, api_key_name: str):
    ...
    return (
        ...
        profile_image,
        identity_str, memory_str, notepad_content, creative_notes_content, research_notes_content, load_system_prompt_content(room_name),
        core_memory_content,
...
def handle_initial_load(room_name: str = None, expected_count: int = 182):
    """
    【v11: 時間デフォルト対応版】
...
def handle_create_room(new_room_name: str, new_user_display_name: str, new_agent_display_name: str, new_room_description: str, initial_system_prompt: str):
    ...
    try:
        ...
        return (
            updated_room_list, updated_room_list, updated_room_list, updated_room_list, gr.update(),
            gr.update(visible=False), 
            gr.update(value=f"✅ ルーム「{new_room_name}」を作成しました。"),
...
def handle_delete_room(confirmed: str, folder_name_to_delete: str, api_key_name: str, current_room_name: str = None, expected_count: int = 156):
    """
    【v7: 引数順序修正版】
...
def handle_translate_thought(
    ...
    try:
        ...
        # 推論用APIキーの取得
        current_api_key = config_manager.get_active_gemini_api_key(model_name=constants.INTERNAL_PROCESSING_MODEL)

        translated_texts = []
        
        # 複数の思考ログがある場合、それぞれ翻訳
        for thought_text in thought_texts:
            translated_part = gemini_api.translate_thought_log_with_ai(thought_text, agent_name, current_api_key)
            if translated_part:
...
def handle_update_episodic_memory(room_name: str, api_key_name: str):
    ...
    yield (
        ...
        gr.update(interactive=True, placeholder="昨日までの会話ログを日ごとに要約して保存します"), 
...
def handle_room_change_for_all_tabs(room_name: str, api_key_name: str, expected_count: int = 156):
    """
    【v11: 最終契約遵守版】
