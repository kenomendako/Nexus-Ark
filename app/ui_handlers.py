...
def handle_initial_load(room_name: str = None, expected_count: int = 205, request: gr.Request = None):
    """
    【v11: 時間デフォルト対応版】
...
def handle_internal_category_change(category: str, current_model: str = None):
    """
...
def handle_load_twitter_draft_by_id(draft_id: str):
    ...
    if draft:
        ...
        return draft["id"], draft["filtered_content"], warnings_text, reply_preview, reply_url, reply_id, media_paths, media_paths
...
def handle_save_log_backup_rotation_count(count: int):
    ...
    int_count = int(count)
    if config_manager.save_config_if_changed("log_backup_rotation_count", int_count):
        gr.Info(f"会話ログのバックアップ保存件数を {int_count} 件に設定しました。")

def handle_periodic_backup_interval_change(interval_str: str):
    """定期バックアップ間隔を変更する"""
    try:
        interval = int(interval_str)
        if config_manager.save_config_if_changed("periodic_backup_interval", interval):
            if interval > 0:
                hours = interval // 3600
                gr.Info(f"定期バックアップ間隔を {hours}時間ごと に設定しました。")
            else:
                gr.Info("定期バックアップを無効化しました。")
    except ValueError:
        pass
...
def handle_user_generate_image(prompt, provider, model, profile_name, room_name, api_key_name):
    ...
    if not prompt or not prompt.strip():
        return gr.update(), gr.update(), gr.update(), "【エラー】プロンプトを入力してください。"
    
    if not room_name:
        return gr.update(), gr.update(), gr.update(), "【エラー】ルームを選択してください。"
    ...
    try:
        ...
        return gr.update(), gr.update(), gr.update(), f"【エラー】生成に失敗しました。\n{result}"

    except Exception as e:
        traceback.print_exc()
        return gr.update(), gr.update(), gr.update(), f"【エラー】予期せぬエラーが発生しました: {str(e)}"
