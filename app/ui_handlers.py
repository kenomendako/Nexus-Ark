...
def handle_initial_load(room_name: str = None, expected_count: int = 199):
    """
    【v11: 時間デフォルト対応版】
...
def handle_save_identity(room_name: str, content: str):
    ...
    try:
        # バックアップ作成
        create_backup(room_name, backup_type='memory')
...
def handle_room_change_for_all_tabs(room_name: str, api_key_val: str, theme_enabled: bool, expected_count: int = 172):
    """
    【v11: 最終契約遵守版】
...
def handle_provider_change(provider_choice: str):
    ...
    is_zhipu = (provider_id == "zhipu")
    is_openai = (provider_id == "openai")
...
def handle_save_internal_model_settings(
    ...
    Args:
        ...
        translation_provider: 思考ログ翻訳モデルのプロバイダ
        translation_model: 思考ログ翻訳モデル名
    ...
    try:
        settings = {
            ...
            # 思考ログ翻訳モデル設定
            "translation_provider": translation_provider or "google",
            "translation_model": (translation_model.strip() if translation_model else constants.INTERNAL_PROCESSING_MODEL),
...
def handle_image_gen_provider_change(provider: str):
    ...
    Returns:
        (gemini_section_visible, openai_section_visible, api_key_visible)
    """
def handle_image_gen_provider_change(provider: str):
    """
    画像生成プロバイダが変更されたときにUIの表示を更新する。
...
def handle_save_food_item(room_name, name, category, amount, image_path, 
    ...
    if not name:
        return [gr.update(value="保存エラー: アイテム名がありません", visible=True), gr.update(), gr.update(), gr.update()]
    ...
    try:
        ...
        if success:
            ...
            choices = [f"{row['アイテム名']} (x{row['所持数']}) [{row['ID']}]" for _, row in food_df.iterrows()]
            choices.insert(0, "(なし)")
            return gr.update(value=f"保存しました: {name} x{int(amount)}", visible=True), gr.update(value=food_df), gr.update(value=std_df), gr.update(choices=choices)
        else:
             return gr.update(value="保存に失敗しました", visible=True), gr.update(), gr.update(), gr.update()
    except Exception as e:
        import traceback
        traceback.print_exc()
        return gr.update(value=f"エラー: {str(e)}", visible=True), gr.update(), gr.update(), gr.update()
...
def handle_refresh_food_inventory(room_name):
    """食べ物と通常アイテム両方のインベントリUIを更新する"""
    food_df, std_df, choices = _get_food_inventory_data(room_name)
    return gr.update(value=food_df), gr.update(value=std_df), gr.update(choices=choices)
...
def handle_manual_refresh_inventory(room_name):
    ...
    # 戻り値: status, food_df, std_df, dropdown_choices
    return gr.update(value=msg, visible=True), food_df, std_df, gr.update(value="(なし)", choices=choices)
...
def handle_save_std_item(room_name, name, category, amount, base_info, image_path, app_desc, app_color, app_design, texture, weight, temp, flavor_text, raw_json):
    """通常アイテムを保存する"""
    if not name or not name.strip():
        return gr.update(value="アイテム名を入力してください", visible=True), gr.update(), gr.update(), gr.update()
    ...
    try:
        ...
        success = im.create_item(item_data, is_user_creator=True, image_path=image_path)
        if success:
            food_df, std_df, choices = handle_refresh_food_inventory(room_name)
            return gr.update(value=f"保存しました: {name} x{int(amount)}", visible=True), food_df, std_df, choices
        else:
             return gr.update(value="保存に失敗しました", visible=True), gr.update(), gr.update(), gr.update()
    except Exception as e:
...
def handle_food_attach(choice_str, room_name):
    """アイテムを相手に贈る(添付)"""
    if not choice_str or choice_str == "(なし)": return gr.update(value="アイテムを選択してください", visible=True), gr.update(), gr.update(), gr.update()
    if not room_name: return gr.update(value="チャット相手(Persona)がいません", visible=True), gr.update(), gr.update(), gr.update()
    
    item_id = _extract_id_from_choice(choice_str)
    if not item_id: return gr.update(value="アイテムIDが不正です", visible=True), gr.update(), gr.update(), gr.update()
    ...
    try:
         ...
         if success:
             ...
             try:
                 import action_logger
                 import utils
                 action_logger.append_action_log(room_name, "system_event", {"event": "item_transfer"}, f"ユーザーから「{item_name}」を受け取りました。使用前には必ずlist_my_itemsでIDを確認してください。")
                 utils.append_system_message_to_log(room_name, f"【システム通知】ユーザーがアイテム「{item_name}」をあなたに贈りました。\n※使用・消費する場合はインベントリ（list_my_items）を確認してください。")
             except Exception as e:
                 print(f"Error logging item transfer: {e}")
                 
             log_msg = f"※ システムログ: あなたはアイテム「{item_name}」をペルソナ({room_name})に贈りました。"
             
             return (
                 gr.update(value=log_msg, visible=True), 
                 gr.update(value=food_df), 
                 gr.update(value=std_df), 
                 gr.update(value="(なし)", choices=choices)
             )
         else:
             return gr.update(value="添付失敗(在庫不足など)", visible=True), gr.update(), gr.update(), gr.update()
    except Exception as e:
         return gr.update(value=f"エラー: {e}", visible=True), gr.update(), gr.update(), gr.update()

def handle_food_consume(choice_str, room_name):
    """自分でアイテムを消費する"""
    if not choice_str or choice_str == "(なし)": return [gr.update(value="アイテムを選択してください", visible=True)] + [gr.update()] * 5
    
    item_id = _extract_id_from_choice(choice_str)
    if not item_id: return [gr.update(value="アイテムIDが不正です", visible=True)] + [gr.update()] * 5
    try:
         from src.features.item_manager import ItemManager
         im = ItemManager(room_name)
         # 削除前にデータ取得して表示テキスト生成
         ...
         if success:
             ...
             return (
                 ...
                 food_df,                                        # food_inventory_df
                 std_df,                                         # std_inventory_df
             ...
             return [gr.update(value="消費に失敗しました", visible=True)] + [gr.update()] * 5
    except Exception as e:
         return [gr.update(value=f"エラー: {e}", visible=True)] + [gr.update()] * 5
...
def handle_place_item_button_click(room_name, location_name, item_choice, amount=1, furniture_name=""):
    ...
    if not item_choice or item_choice == "(なし)": 
        return gr.update(value="置くアイテムを選択してください", visible=True), gr.update(), gr.update(), gr.update(), gr.update(), gr.update()
    
    item_id = _extract_id_from_choice(item_choice)
    if not item_id: 
        return gr.update(value="アイテムIDの抽出に失敗しました", visible=True), gr.update(), gr.update(), gr.update(), gr.update(), gr.update()
    ...
    try:
        ...
        if success:
            food_df, std_df, inventory_choices = _get_food_inventory_data(room_name)
            ...
            # 戻り値: status, food_df, std_df, inventory_dropdown, location_item_dropdown, furniture(reset)
            return (
                gr.update(value=msg, visible=True), 
                gr.update(value=food_df), 
                gr.update(value=std_df), 
                gr.update(value="(なし)", choices=inventory_choices), 
                gr.update(value="(なし)", choices=loc_choices), 
                gr.update(value="")
            )
        else:
            return gr.update(value="配置に失敗しました", visible=True), gr.update(), gr.update(), gr.update(), gr.update(), gr.update()
    except Exception as e:
        return gr.update(value=f"エラー: {e}", visible=True), gr.update(), gr.update(), gr.update(), gr.update(), gr.update()

def handle_pickup_item_button_click(room_name, location_name, item_choice, amount=1):
    """場所にあるアイテムを拾う（ドロップダウン対応）"""
    if not item_choice or item_choice == "(なし)": 
        return gr.update(value="拾うアイテムを選択してください", visible=True), gr.update(), gr.update(), gr.update(), gr.update()
    ...
    try:
        ...
        if success:
            food_df, std_df, inv_choices = _get_food_inventory_data(room_name)
            loc_choices = _get_location_items_choices(room_name, location_name)
            ...
            return (
                ...
                gr.update(value=food_df), 
                gr.update(value=std_df), 
                gr.update(value="(なし)", choices=inv_choices), 
                gr.update(value="(なし)", choices=loc_choices)
            )
        else:
            return gr.update(value="拾得に失敗しました", visible=True), gr.update(), gr.update(), gr.update(), gr.update()
    except Exception as e:
...
def handle_consume_location_item_button_click(room_name, location_name, item_choice, amount=1):
    ...
    if not item_choice or item_choice == "(なし)": 
        return [gr.update(value="消費するアイテムを選択してください", visible=True)] + [gr.update()] * 5
...
def handle_delete_inventory_item(room_name, confirm_val, item_choice, food_sel=None, std_sel=None):
    """インベントリのアイテムを完全に削除。作成画面での選択セレクタ(State)も考慮する。"""
    if not confirm_val: return gr.update(value="削除をキャンセルしました", visible=True), gr.update(), gr.update(), gr.update()
    item_id = None
    
    # 1. 選択ドロップダウンから取得
    if item_choice and item_choice != "(なし)":
        item_id = _extract_id_from_choice(item_choice)
    
    # 2. ドロップダウンで選んでいない場合、作成画面の選択 State から取得を試みる
    if not item_id:
        if food_sel is not None:
            df = _get_food_inventory_df(room_name)
            if food_sel < len(df): item_id = df.iloc[food_sel]["ID"]
        elif std_sel is not None:
            df = _get_std_inventory_df(room_name)
            if std_sel < len(df): item_id = df.iloc[std_sel]["ID"]

    if not item_id: return gr.update(value="削除するアイテムを選択してください", visible=True), gr.update(), gr.update(), gr.update()
    
    try:
        from src.features.item_manager import ItemManager
        im = ItemManager(room_name)
        success = im.delete_item(item_id, is_user=True)
        if success:
            food_df, std_df, choices = handle_refresh_food_inventory(room_name)
            return gr.update(value=f"アイテムを削除しました", visible=True), food_df, std_df, choices
        else:
            return gr.update(value="削除に失敗しました", visible=True), gr.update(), gr.update(), gr.update()
    except Exception as e:
        return gr.update(value=f"エラー: {e}", visible=True), gr.update(), gr.update(), gr.update()
...
def handle_copy_inventory_item(room_name, item_choice):
    """インベントリのアイテムを複製"""
    if not item_choice or item_choice == "(なし)": return gr.update(value="コピーするアイテムを選択してください", visible=True), gr.update(), gr.update(), gr.update()
    
    item_id = _extract_id_from_choice(item_choice)
    if not item_id: return gr.update(value="アイテムIDが見つかりません", visible=True), gr.update(), gr.update(), gr.update()
    ...
    try:
        ...
        if success:
            food_df, std_df, choices = handle_refresh_food_inventory(room_name)
            return gr.update(value=f"アイテムをコピーしました", visible=True), food_df, std_df, choices
        else:
            return gr.update(value="コピーに失敗しました", visible=True), gr.update(), gr.update(), gr.update()
    except Exception as e:
        return gr.update(value=f"エラー: {e}", visible=True), gr.update(), gr.update(), gr.update()
