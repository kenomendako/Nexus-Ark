...
def handle_save_nim_key(api_key: str):
    ...
    if config_manager.save_config_if_changed("nim_api_key", api_key):
        ...
        gr.Info("Nvidia NIM APIキーを保存し、OpenAI互換プロファイルを登録しました。")
    else:
...
def handle_save_xai_key(api_key: str):
    ...
    if config_manager.save_config_if_changed("xai_api_key", api_key):
        ...
        gr.Info("X.ai APIキーを保存し、OpenAI互換プロファイルを登録しました。")
    else:
...
def handle_initial_load(room_name: str = None, expected_count: int = 192):
    """
    【v11: 時間デフォルト対応版】
...
def handle_message_submission(
    ...
    if not full_user_log_entry:
        ...
        # 戻り値の数を16個に合わせる
        yield (history, mapping, gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(visible=False), gr.update(interactive=True), gr.update(), gr.update(), translation_cache)
        return
...
def handle_delete_room(confirmed: str, folder_name_to_delete: str, api_key_name: str, current_room_name: str = None, expected_count: int = 166):
    """
    【v7: 引数順序修正版】
    ...
    try:
        ...
        if new_room_list:
            ...
            empty_chat_updates = (
                ...
                gr.update(value="00:00"),  # room_quiet_hours_start
                gr.update(value="07:00"),  # room_quiet_hours_end
                *[gr.update()]*11, # room_model_dropdown to room_rotation_dropdown (11 items)
                *[gr.update()]*6,  # roblox_api_key_input to roblox_webhook_secret_input
...
def handle_reload_working_memory(room_name: str, slot_name: str = None) -> str:
    """ワーキングメモリを再読み込み"""
    if not room_name:
        gr.Warning("ルームが選択されていません。")
        return ""
    content = load_working_memory_content(room_name, slot_name)
    gr.Info(f"「{room_name}」のワーキングメモリを再読み込みしました。")
    return content
...
def handle_room_change_for_all_tabs(room_name: str, api_key_val: str, theme_enabled: bool, expected_count: int = 166):
    """
    【v11: 最終契約遵守版】
