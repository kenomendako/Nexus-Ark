...
def _convert_room_setting_delta(field: str, value: Any) -> Tuple[str, Any, str]:
    ...
    if field == "voice_id":
        ...
        value = next((k for k, v in constants.API_HISTORY_LIMIT_OPTIONS.items() if v == value), "all")
        ...
        value = None if value == "default" else value
...
def handle_save_room_setting_delta(room_name: str, field: str, value: Any, is_switching_room: bool = False) -> str:
    """ルーム個別設定を1キーだけ差分保存する。"""
    if _should_skip_auto_settings_save(is_switching_room):
        return gr.update()
    if not room_name:
...
def load_roblox_guide():
    ...
    guide_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "guides", "roblox_quickstart_guide.md")
    try:
...
def handle_initial_load(room_name: str = None, expected_count: int = 208, request: gr.Request = None):
    """
    【v11: 時間デフォルト対応版】
    ...
    if has_valid_key:
        token_count_text = "トークン数: 計算待ち"
        t_step = _perf_log("handle_initial_load: token calculation deferred", t_step)
        onboarding_guide_update = gr.update(visible=False)
        chat_input_update = gr.update(interactive=True)
    else:
        t_step = _perf_log("handle_initial_load: token calculation skipped", t_step)
...
def handle_initial_chat_load(room_name: str = None, request: gr.Request = None):
    ...
    token_count_text = "トークン数: 計算待ち" if has_valid_key else "トークン数: (APIキー未設定)"
...
def _get_room_settings_fast_updates(room_name: str, api_key_name: str = None):
    ...
    voice_id = effective_settings.get("voice_id")
    voice_name = config_manager.SUPPORTED_VOICES.get(voice_id, next(iter(config_manager.SUPPORTED_VOICES.values()), ""))
    ...
    return (
        ...
        gr.update(value=effective_settings.get("voice_style_prompt", "")),
        ...
        gr.update(value=effective_settings.get("streaming_speed", 0.01)),
...
def handle_delete_room(confirmed: str, folder_name_to_delete: str, api_key_name: str, current_room_name: str = None, expected_count: int = 175):
    """
    【v7: 引数順序修正版】
...
def _parse_log_content_parts(content: str, remove_thoughts: bool = False) -> List[Dict[str, str]]:
    """
    ログ本文を通常本文と思考ログに分解する。
    ...
    content = utils.clean_persona_text(content, remove_thoughts=remove_thoughts)
    if remove_thoughts:
    ...
    for match in _PAIRED_THOUGHT_RE.finditer(content):
        ...
        if full_match.upper().startswith("[THOUGHT]"):
            style = "new"
            thought = match.group(2)
        elif full_match.startswith("【Thoughts】"):
            style = "legacy"
...
def format_history_for_gradio(
    ...
    for item in proto_history:
        ...
        if item["type"] == "text":
            ...
            parsed_parts = _parse_log_content_parts(content_to_parse, remove_thoughts=not display_thoughts)
            if display_thoughts:
...
def handle_notification_service_change(service_choice: str):
    if service_choice in ["Discord", "Pushover"]:
        ...
        return handle_save_global_setting_delta("notification_service", service_value, f"通知サービス「{service_choice}」", skip_grace=True)
    return gr.update()
...
def handle_full_reindex(room_name: str, api_key_name: str):
    ...
    try:
        ...
        # 続いて現行ログの索引も更新する
        yield f"現行ログの索引を更新中...", gr.update(interactive=False)
        for b_num, b_total, status_msg in manager.update_current_log_index_with_progress():
            last_message = status_msg
            yield f"現行ログ更新: {status_msg}", gr.update(interactive=False)

        gr.Info(f"✅ インデックス（記憶・現行ログ）の完全再構築が完了しました")
        last_updated = _get_rag_index_last_updated(room_name, "memory")
...
def handle_internal_embedding_provider_change(provider: str):
    ...
    if provider == "google":
        choices = [
            ("gemini-embedding-2-preview (最新・推奨)", "gemini-embedding-2-preview"),
        ...
        default_val = current if current in ["gemini-embedding-2-preview", "gemini-embedding-001"] else "gemini-embedding-2-preview"
    ...
    try:
        ...
        if os.path.exists(log_path) and os.path.getsize(log_path) > 0:
            ...
            if not raw_content or not str(raw_content).strip():
                gr.Error(
                    f"安全のため、非空の会話ログを空内容で上書きする保存を中止しました ({os.path.basename(log_path)})"
                )
                return (
                    gr.update(value=existing_content),
                    gr.update(),
                    gr.update(),
                    gr.update()
                )
...
def handle_save_food_item(room_name, name, category, amount, image_path,
    ...
    if not name or not str(name).strip():
        ...
        gr.Warning(msg)
        return gr.update(value=msg, visible=True), gr.update(), gr.update(), gr.update(value=msg, visible=True)
...
def handle_refresh_twitter_pending():
    ...
    if not pending:
        return pd.DataFrame(columns=["ID", "時刻", "画像", "下書き内容", "警告"])
    ...
    return pd.DataFrame(data, columns=["ID", "時刻", "画像", "下書き内容", "警告"])
...
def handle_refresh_twitter_all(room_name):
    ...
    # 3. 下書き取得
    drafts = twitter_manager.get_pending_list()
    draft_data = []
    for d in drafts:
        draft_data.append([
            d["id"],
    ...
    return [session_placeholder] + settings + [draft_df, history_df]
...
def update_user_gen_model_choices(provider, profile_name):
    ...
    if is_openai:
        ...
        model_update, visibility_update = handle_user_gen_profile_change(profile_name)
        return model_update, gr.update(visible=True), visibility_update
...
def handle_user_gen_profile_change(profile_name):
    ...
    if models:
        return gr.update(choices=models, value=models[0]), gr.update(visible=is_openrouter)

    # プロファイル名に pollinations が含まれる場合は Pollinations のモデルリストを表示
    if "pollinations" in profile_name.lower():
        models = config_manager.CONFIG_GLOBAL.get("available_image_models", {}).get("pollinations", [])
    else:
        # デフォルトの OpenAI 画像モデル
        models = config_manager.CONFIG_GLOBAL.get("available_image_models", {}).get("openai", [])
...
"""
        ...
        effective_settings = config_manager.get_effective_settings(room_name)
        # 内部処理用のロール指定
        generator_llm = LLMFactory.create_chat_model(
            api_key=None,
            generation_config=effective_settings,
            internal_role="processing"
        )
        ...
        # 実行
        response = generator_llm.invoke(system_prompt)
...
def handle_approve_dependency(pkg_name: str):
    ...
    gr.Info(f"パッケージ '{pkg_name}' を承認しました。")
...
def handle_accept_disclaimer(service_name: str):
    ...
    # 1番目: ガードパネルを非表示, 2番目: メインコンテンツを表示
