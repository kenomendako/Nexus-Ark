...
class TwitterManager:
    ...
    def reload(self):
        """JSONファイルから最新のデータを再読み込みする"""
        self.pending_tweets = self._load_json(PENDING_TWEETS_FILE, [])
        self.tweet_history = self._load_json(TWEET_HISTORY_FILE, [])
        logger.debug("Twitter pending/history data reloaded from disk.")
    ...
    def _get_room_data_dir(self, room_name: Optional[str]) -> str:
        safe_room = str(room_name or "global").replace("/", "_").replace("\\", "_")
        return os.path.join(TWITTER_DATA_DIR, safe_room)

    def _get_cookies_path(self, room_name: Optional[str]) -> str:
        if not room_name:
            return COOKIES_FILE
        room_cookie_path = os.path.join(self._get_room_data_dir(room_name), "cookies.json")
        if not os.path.exists(room_cookie_path) and os.path.exists(COOKIES_FILE):
            try:
                os.makedirs(os.path.dirname(room_cookie_path), exist_ok=True)
                shutil.copy2(COOKIES_FILE, room_cookie_path)
                logger.info(f"既存Twitter Cookieをルーム別パスへコピーしました: {room_cookie_path}")
            except Exception as e:
                logger.warning(f"Twitter Cookie移行コピーに失敗しました: {e}")
        return room_cookie_path
    ...
    def get_twitter_post_limit(self, room_name: Optional[str] = None) -> int:
        """ルームのPremium設定に応じた投稿文字数上限を返す。"""
        settings = self._get_full_twitter_settings(room_name) if room_name else {}
        return 25000 if settings.get("is_premium", False) else 280
    ...
    def apply_privacy_filter(self, text: str, room_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        limit = self.get_twitter_post_limit(room_name)
        warn_limit = 24900 if limit > 280 else 260
    ...
    def add_draft(self, content: str, room_name: str, author_type: str = "persona", reply_to_url: Optional[str] = None, reply_to_id: Optional[str] = None, media_paths: Optional[List[str]] = None) -> str:
        """
        承認待ちキューに下書きを追加する。
        """
    ...
    def approve_tweet(self, draft_id: str, edited_content: Optional[str] = None, edited_reply_url: Optional[str] = None, edited_media_paths: Optional[List[str]] = None) -> bool:
        """
        下書きを承認する。
        """
        for i, draft in enumerate(self.pending_tweets):
            if draft["id"] == draft_id:
                ...
                if edited_media_paths is not None:
                    target["media_paths"] = edited_media_paths
        ...
        try:
            ...
            for item in items:
                ...
                if use_api and api_client and tweet_id:
                    ...
                    thread_data = run_get_tweet_thread(self._get_cookies_path(room_name), url, max_depth)
    ...
    def fetch_timeline(self, room_name: str, count: int = 10) -> List[Dict[str, Any]]:
        ...
        if use_api:
            ...
            res = run_get_timeline(self._get_cookies_path(room_name), count)
    ...
    def fetch_mentions(self, room_name: str, count: int = 10) -> List[Dict[str, Any]]:
        ...
        if use_api:
            ...
            res = run_get_mentions(self._get_cookies_path(room_name), count)
    ...
    def fetch_notifications(self, room_name: str, count: int = 10) -> List[Dict[str, Any]]:
        ...
        if not use_api:
            ...
            res = run_get_notifications(self._get_cookies_path(room_name), count)
        else:
            # APIモード時はメンション取得で代用
    ...
    def filter_new_tweets(self, room_name: str, feed_type: str, tweets: List[Dict]) -> List[Dict]:
        """取得したツイートのリストから、前回取得時より新しいものだけを抽出し、既読状態を更新する。"""
        if not tweets:
            return []
            
        state_file = os.path.join(self._get_room_data_dir(room_name), "twitter_state.json")
        state = self._load_json(state_file, {})
        last_seen = state.get("last_seen_ids", {})
        
        last_id_str = last_seen.get(feed_type)
        ...
        for tweet in tweets:
            tid_str = tweet.get("id")
            if not tid_str or not str(tid_str).isdigit():
                # IDがない、または数値でない場合はそのまま返す
        ...
        # 状態更新
        if max_id > last_id:
            if "last_seen_ids" not in state:
                state["last_seen_ids"] = {}
            state["last_seen_ids"][feed_type] = str(max_id)
    ...
    def is_logged_in(self, room_name: Optional[str] = None) -> bool:
        """現在のセッションが有効か確認する"""
        from twitter_browser import run_check_session
        return run_check_session(self._get_cookies_path(room_name))

    def start_login(self, room_name: Optional[str] = None) -> bool:
        """ログインUI（ブラウザ）を起動する"""
        from twitter_browser import run_login_ui
        return run_login_ui(self._get_cookies_path(room_name))

    def import_cookies(self, cookies_json: str, room_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # 保存
            self._save_json(self._get_cookies_path(room_name), cookies)
    ...
    def execute_post(self, draft_id: str, room_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if use_api:
            ...
            media_paths = target_tweet.get("media_paths", [])
            ...
            if media_paths:
                for path in media_paths:
                    if os.path.exists(path):
                        m_id = api.upload_media(path)
                        if m_id:
                            media_ids.append(m_id)
            
            tweet_id = api.post_tweet(content, in_reply_to_tweet_id=reply_to_id, media_ids=media_ids if media_ids else None)
            if tweet_id:
            ...
            try:
                ...
                media_paths = target_tweet.get("media_paths", [])
                result = run_post_tweet(self._get_cookies_path(room_name), content, reply_to_url=reply_to_url, media_paths=media_paths)
