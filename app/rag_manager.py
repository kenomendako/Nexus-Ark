...
def _parse_episodic_date_range(metadata: Dict) -> Optional[Tuple[date, date]]:
    """
    episodic_memory の date メタデータからカバー日付範囲を取り出す。
    単日 YYYY-MM-DD とレンジ YYYY-MM-DD~YYYY-MM-DD のみを対象にする。
    """
    if metadata.get("type") != "episodic_memory":
        return None
    ...
    parts = [part.strip() for part in raw_date.split("~", 1)]
    try:
        if len(parts) == 1:
...
def _date_ranges_overlap(left: Tuple[date, date], right: Tuple[date, date]) -> bool:
    return left[0] <= right[1] and right[0] <= left[1]


ScoredRAGResult = Tuple[Document, float, float, float, float]


def _collect_dream_insight_documents(memory_dir: Path, processed_records: Set[str]) -> List[Tuple[str, Document]]:
    """
    夢日記をRAG用Documentへ変換する。
    現行の月次保存先 memory/dreaming/*.json を優先し、レガシー insights.json も存在時のみ読む。
    """
    pending_items: List[Tuple[str, Document]] = []
    dream_files: List[Tuple[Path, str]] = []
    seen_record_ids = set(processed_records)

    dreaming_dir = memory_dir / "dreaming"
    if dreaming_dir.exists():
        dream_files.extend((path, f"dreaming/{path.name}") for path in sorted(dreaming_dir.glob("*.json")))

    legacy_insights_path = memory_dir / "insights.json"
    if legacy_insights_path.exists():
        dream_files.append((legacy_insights_path, "insights.json"))

    for dream_path, source_label in dream_files:
        try:
            ...
            for item in insights:
                ...
                if record_id not in seen_record_ids:
                    ...
                    if insight_content:
                        ...
                        doc = Document(page_content=content, metadata={"source": source_label, "type": "dream_insight", "date": date_str})
...
def _collect_diary_documents(memory_dir: Path, processed_records: Set[str]) -> List[Tuple[str, Document]]:
    """日記ファイルをRAG用Documentへ変換する。memory_identity.txt は専用identity索引へ分離する。"""
    pending_items: List[Tuple[str, Document]] = []
    if not memory_dir.exists():
        return pending_items

    for f in memory_dir.glob("memory*.txt"):
        if f.name == "memory_identity.txt":
            continue
...
def _collect_identity_documents(memory_dir: Path, processed_records: Set[str]) -> Tuple[List[Tuple[str, Document]], Dict[str, str]]:
    """memory_identity.txt を専用identity型としてRAG用Documentへ変換し、現行ハッシュを返す。"""
    pending_items: List[Tuple[str, Document]] = []
    current_hashes: Dict[str, str] = {}
    identity_path = memory_dir / "memory_identity.txt"
    if not identity_path.exists():
        return pending_items, current_hashes
    ...
    try:
        ...
        content_hash = hashlib.md5(content.encode()).hexdigest()[:8]
        current_hashes[identity_path.name] = content_hash
        record_id = f"identity:{identity_path.name}:{content_hash}"
        if record_id not in processed_records:
            doc = Document(
                page_content=content,
                metadata={
                    "source": identity_path.name,
                    "type": "identity",
        ...
        print(f"  - 永続記憶ファイル読み込みエラー ({identity_path.name}): {e}")

    return pending_items, current_hashes


def _dedupe_scored_rag_results(
    scored_results: List[ScoredRAGResult],
    current_identity_hashes: Optional[Dict[str, str]] = None,
) -> Tuple[List[ScoredRAGResult], int, int, int]:
    """
    RAG検索結果をスコア順のまま先勝ちで重複除去する。
    既存の先頭100文字content dedupに加え、episodic_memoryは日付レンジが重なるものを除外する。
    identity は現行ハッシュ以外、および旧diary型で残った memory_identity.txt を検索結果から除外する。
    """
    seen_contents = set()
    seen_episodic_ranges: List[Tuple[date, date]] = []
    unique_results = []
    content_duplicate_count = 0
    episodic_duplicate_count = 0
    identity_stale_count = 0
    identity_hashes = current_identity_hashes or {}

    for doc, sim_score, arousal, decay, comp_score in scored_results:
        source = doc.metadata.get("source")
        doc_type = doc.metadata.get("type")
        current_identity_hash = identity_hashes.get(source)
        if current_identity_hash:
            if doc_type != "identity" or doc.metadata.get("identity_hash") != current_identity_hash:
        ...
        episodic_range = _parse_episodic_date_range(doc.metadata)
        if episodic_range is not None and any(
            _date_ranges_overlap(episodic_range, seen_range)
            for seen_range in seen_episodic_ranges
        ):
            episodic_duplicate_count += 1
            continue

        seen_contents.add(content_key)
        if episodic_range is not None:
            seen_episodic_ranges.append(episodic_range)
        unique_results.append((doc, sim_score, arousal, decay, comp_score))

    return unique_results, content_duplicate_count, episodic_duplicate_count, identity_stale_count
...
class RAGManager:
    ...
    _model_mismatch_notices: Set[str] = set()
    INDEX_LABELS = {
        "faiss_index_static": "記憶索引",
        "faiss_index_dynamic": "ナレッジ索引",
        "current_log_index": "現行ログ索引",
        "faiss_index": "旧形式の記憶索引",
    }
    INDEX_DATA_FILENAMES = ("index.faiss", "index.pkl", "docstore.json")
    ...
    def __init__(self, room_name: str, api_key: str):
        ...
        self.rotation_session_revivals = 0
        ...
        self.identity_current_record = self.rag_data_dir / "identity_current.json"
        self.last_rebuild_result: Dict[str, Any] = {
            "success": False,
            "message": "完全再構築はまだ実行されていません。",
            "errors": [],
        }
        ...
        embedding_mode = effective_settings.get("embedding_mode", "api")
        self.embedding_mode = embedding_mode if embedding_mode in ("local", "api") else "api"

        # [2026-06-23 FIX] エンベディングは内部処理。共通設定のキー/ローテーションに従い、
        # ルーム個別の有料キー等を波及させない（最終応答のみ個別設定に従う方針と一貫）。
        # Gemini API エンベディング時のみ対象（local/OpenAIは自前の設定を使う）。
        if self.embedding_mode == "api":
            try:
                common_key = config_manager.get_active_gemini_api_key(None)
                if common_key:
                    self.api_key = common_key
            except Exception:
                pass
    ...
    def _reset_rotation_session(self) -> None:
        """RAG内のキー試行セッション状態を初期化する。"""
    ...
    def _index_label(self, target_path: Path) -> str:
        """索引パスをユーザー向けの名称へ変換する。"""
        return self.INDEX_LABELS.get(Path(target_path).name, Path(target_path).name)

    def inspect_index_compatibility(self, target_path: Path) -> Dict[str, Any]:
        """索引実体とmodel_info.jsonを確認し、現在設定との互換性を返す。"""
        target_path = Path(target_path)
        current_model_id = self._get_embedding_model_id()
        result: Dict[str, Any] = {
            "path": str(target_path),
            "label": self._index_label(target_path),
        ...
        has_index_data = any(
            (target_path / filename).exists()
            for filename in self.INDEX_DATA_FILENAMES
        )
        if not has_index_data:
            # 空ディレクトリや model_info.json だけの準備状態は「索引未作成」。
            # 記憶システムを使っていないルームへ不一致警告を出さない。
            result["reason"] = "index_not_created"
            return result
        ...
        if not model_info_path.exists():
            result.update(compatible=False, reason="model_info_missing")
            return result
        ...
        try:
            ...
            result.update(compatible=False, reason="model_info_invalid")
            return result

        saved_model_id = info.get("model_id") if isinstance(info, dict) else None
        result["saved_model_id"] = saved_model_id
        if not saved_model_id:
            result.update(compatible=False, reason="model_id_missing")
        elif saved_model_id != current_model_id:
            result.update(compatible=False, reason="model_mismatch")
        else:
            result["reason"] = "compatible"
        return result

    def _notify_incompatible_index(self, inspection: Dict[str, Any]) -> None:
        """同じ不一致をプロセス中1回だけ、索引名付きで通知する。"""
        if inspection.get("compatible") or not inspection.get("exists"):
            return

        label = inspection.get("label", "RAG索引")
        saved_model_id = inspection.get("saved_model_id") or "不明（モデル情報なし）"
        current_model_id = inspection.get("current_model_id") or "不明"
        reason = inspection.get("reason", "unknown")
        signature = "|".join(
            [
                self.room_name,
                str(Path(inspection.get("path", "")).resolve()),
                str(saved_model_id),
                str(current_model_id),
                str(reason),
            ]
        )
        if signature in self._model_mismatch_notices:
            return
        self._model_mismatch_notices.add(signature)

        msg = (
            f"RAG警告: {label}のモデル（{saved_model_id}）と現在の設定（{current_model_id}）が一致しません。"
            "旧索引への追記と検索を停止しました。記憶・ノートの「🗑️ 索引を初期化して再構築」を実行してください。"
        )
        utils.add_system_notice(msg, level="warning")
        print(f"--- [RAG Model Guard] {msg} ---")

    def _incremental_update_block_message(self, target_path: Path) -> Optional[str]:
        """既存索引が現在モデルと非互換なら差分更新を拒否する。"""
        inspection = self.inspect_index_compatibility(target_path)
        if not inspection.get("exists") or inspection.get("compatible"):
            return None
        self._notify_incompatible_index(inspection)
        return (
            f"⚠️ {inspection['label']}: 既存索引のモデルが現在設定と一致しないため更新を中止しました。"
            "「🗑️ 索引を初期化して再構築」を実行してください。"
        )

    def validate_existing_index_models(self) -> List[str]:
        """存在する全索引が現在モデルで読める状態か検証し、問題一覧を返す。"""
        errors: List[str] = []
        paths = [
            self.rag_data_dir / "faiss_index_static",
            self.rag_data_dir / "faiss_index_dynamic",
            self.rag_data_dir / "current_log_index",
            self.rag_data_dir / "faiss_index",
        ]
        for path in paths:
            inspection = self.inspect_index_compatibility(path)
            if inspection.get("exists") and not inspection.get("compatible"):
                saved = inspection.get("saved_model_id") or "不明"
                errors.append(
                    f"{inspection['label']}: 保存モデル={saved}, "
                    f"現在モデル={inspection['current_model_id']}, 状態={inspection['reason']}"
                )
        return errors
    ...
    def _resolve_tried_key_ref(self, key_ref: str) -> tuple[str, str] | None:
        """tried_keys 内のキー名/キー値混在を吸収して (key_name, key_value) にする。"""
        if key_ref in config_manager.GEMINI_API_KEYS:
            key_value = config_manager.GEMINI_API_KEYS.get(key_ref)
            return (key_ref, key_value) if key_value else None

        key_name = config_manager.get_key_name_by_value(key_ref)
        if not key_name or key_name == "Unknown":
            return None
        return key_name, key_ref

    def _sleep_for_rotation_revival(self, total_seconds: int = 60) -> None:
        """RPM窓の回復待ち。テストで monkeypatch しやすいよう分離する。"""
    ...
    def _try_revive_rotation_session(self, model_name: str) -> Union[str, bool]:
        """試行済みラチェットから、回復済みキーを最大2回だけ復活させる。"""
        if self.rotation_session_revivals >= 2:
            print("      [RAG Rotation] Session revival limit reached. Giving up this cycle.")
            return False

        paid_keys = set(config_manager.CONFIG_GLOBAL.get("paid_api_key_names", []) or [])
        candidates = []
        for key_ref in list(self.tried_keys.keys()):
            resolved = self._resolve_tried_key_ref(key_ref)
            if not resolved:
                continue
            key_name, key_value = resolved
            if config_manager.is_key_exhausted(key_name, model_name=model_name):
                continue
            candidates.append((0 if key_name in paid_keys else 1, key_name, key_value))

        if not candidates:
            return False

        _, revived_key_name, revived_key_value = sorted(candidates, key=lambda item: (item[0], item[1]))[0]
        self.rotation_session_revivals += 1
        print(
            f"      [RAG Rotation] Reviving tried key '{revived_key_name}' "
            f"after RPM cooldown ({self.rotation_session_revivals}/2)."
        )
        self._sleep_for_rotation_revival(60)
        self.tried_keys.pop(revived_key_name, None)
        self.tried_keys.pop(revived_key_value, None)
        self.api_key = revived_key_value
    ...
    def _get_actual_embeddings(self, task_type="retrieval_document"):
        ...
        if task_type not in self.actual_embeddings:
            raise RuntimeError(
                f"RAGエンベディングの初期化に失敗しました。"
                f"task_type={task_type}, embedding_mode={self.embedding_mode}"
            )
        return self.actual_embeddings[task_type]
    ...
    def _rotate_api_key(self, error_str: str) -> Union[str, bool]:
        ...
        # [2026-06-23 FIX] 無限リトライ防止のため待機回数に上限を設ける。
        # 従来は tried_keys から削除して "waited" を返し続けるため、有料キーが
        # 制限され続けると attempt を増やさない呼び出し側ループと相まって無限ループしていた。
        paid_keys = config_manager.CONFIG_GLOBAL.get("paid_api_key_names", [])
        if key_name in paid_keys:
            wait_count = self.tried_keys.get(self.api_key, 0)
            if wait_count >= 3:
                print(f"      [RAG Rotation] Paid key '{key_name}' still throttled after {wait_count} waits. Giving up this cycle.")
                revived = self._try_revive_rotation_session(clean_model_name)
                if revived:
                    return revived
                return False
            print(f"      [RAG Rotation] Paid key '{key_name}' throttled. Waiting 5s for backoff (wait {wait_count+1}/3)...")
            time.sleep(5)
            self.tried_keys[self.api_key] = wait_count + 1  # 上限カウント（削除しない＝無限化させない）
    ...
    def _load_current_identity_hashes(self) -> Dict[str, str]:
        if self.identity_current_record.exists():
            try:
                with open(self.identity_current_record, 'r', encoding='utf-8') as f:
                    data = json.load(f)
    ...
    def _save_current_identity_hashes(self, current_hashes: Dict[str, str]) -> None:
        try:
            with open(self.identity_current_record, 'w', encoding='utf-8') as f:
                json.dump(current_hashes, f, indent=2, ensure_ascii=False, sort_keys=True)
        except Exception as e:
            print(f"  - identity_current.json 保存エラー: {e}")
    ...
    def classify_query_intent(self, query: str) -> dict:
        ...
        while attempt < max_retries:
            try:
                ...
                # [2026-06-23 FIX] 内部処理は共通設定のキーに従う（個別キーは使わない）。
                # api_key=None で create_chat_model が共通設定のキー/ローテーションを解決する。
...
カテゴリ名のみを1単語で回答してください（emotional/factual/technical/temporal/relational）:"""
    ...
    def _safe_save_index(self, db: FAISS, target_path: Path):
        ...
        with tempfile.TemporaryDirectory(prefix=".tmp_index_", dir=(str(tmp_base_dir) if tmp_base_dir else None)) as temp_dir:
            ...
            # 索引本体とモデル情報を同じ一時ディレクトリ内で完成させてから置換する。
            # 配置後の model_info.json 書き込み失敗で、モデル不明の索引が残ることを防ぐ。
            model_info = {
                "model_id": self._get_embedding_model_id(),
                "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }
            with open(temp_path / "model_info.json", 'w', encoding='utf-8') as f:
    ...
    def update_memory_index(self, status_callback=None) -> str:
        ...
        blocked_message = self._incremental_update_block_message(self.static_index_path)
        if blocked_message:
            if status_callback:
                status_callback(blocked_message)
            print(f"--- [RAG Memory] {blocked_message} ---")
        ...
        # 3. 夢日記収集
        memory_dir = self.room_dir / "memory"
        pending_items.extend(_collect_dream_insight_documents(memory_dir, processed_records))

        # 4. 日記ファイル収集（memory_identity.txt は専用identity型へ分離）
        pending_items.extend(_collect_diary_documents(memory_dir, processed_records))
        identity_items, current_identity_hashes = _collect_identity_documents(memory_dir, processed_records)
        pending_items.extend(identity_items)
        self._save_current_identity_hashes(current_identity_hashes)
    ...
    def update_current_log_index_with_progress(self):
        ...
        blocked_message = self._incremental_update_block_message(current_index_dir)
        if blocked_message:
            yield (0, 0, blocked_message)
            return
    ...
    def search(
        ...
        Args:
            ...
            scope: 検索対象。"all"（記憶＋ナレッジ）、"memory"（静的な記憶索引のみ）、
                   "knowledge"（動的なナレッジ索引のみ）のいずれか。
        """
        # [2026-02-11 FIX] 試行済みキーをリセット
        self._reset_rotation_session()
        ...
        if normalized_scope not in {"all", "memory", "knowledge"}:
            raise ValueError(f"不正なRAG検索スコープです: {scope}")
        ...
        dynamic_db = (
            self._safe_load_index(self.dynamic_index_path)
            if normalized_scope in {"all", "knowledge"}
            else None
        )
        static_db = (
            self._safe_load_index(self.static_index_path)
            if normalized_scope in {"all", "memory"}
            else None
        )
        print(
            f"--- [PERF] RAGManager.search: safe_load_index "
            f"(scope={normalized_scope}) took: {time.time() - load_start:.4f}s ---"
        )
        ...
        # [2026-07-04 追加] episodic_memory の同一日/日付レンジ重複除去
        # [2026-07-08 追加] identity の旧版チャンクと旧diary型チャンクを除去
        current_identity_hashes = self._load_current_identity_hashes()
        unique_results, content_duplicate_count, episodic_duplicate_count, identity_stale_count = _dedupe_scored_rag_results(
            scored_results,
            current_identity_hashes=current_identity_hashes,
        )
        duplicate_count = content_duplicate_count + episodic_duplicate_count + identity_stale_count

        if duplicate_count > 0:
            print(
                f"  - [RAG] 重複除去: {len(scored_results)}件 → {len(unique_results)}件 "
                f"({duplicate_count}件除去: content={content_duplicate_count}, "
                f"episodic_date={episodic_duplicate_count}, identity_stale={identity_stale_count})"
            )
    ...
    def _set_rebuild_failure(self, errors: List[str]) -> None:
        message = "完全再構築に失敗しました: " + " / ".join(errors)
    ...
    def rebuild_all_indices_with_progress(self):
        """全索引を削除・各1回再構築し、モデル整合性確認まで進捗を返す。"""
        self._reset_rotation_session()
        self.last_rebuild_result = {
            "success": False,
            "message": "完全再構築を開始しました。",
            "errors": [],
        }
        yield (0, 4, "既存索引を消去中...")

        # FAISS参照を先に解放し、Windowsでも削除できる状態へ寄せる。
        ...
        paths_to_delete = [
            ...
            self.identity_current_record,
            self.rag_data_dir / "current_log_index",
            self.rag_data_dir / "current_log_meta.json",
        ]
        ...
        for path in paths_to_delete:
            ...
            try:
                ...
                deletion_errors.append(f"{self._index_label(path)}の削除失敗 ({exc})")

        remaining_paths = [str(path) for path in paths_to_delete if path.exists()]
        if remaining_paths:
            deletion_errors.append("削除後も残っている索引: " + ", ".join(remaining_paths))
        if deletion_errors:
            self._set_rebuild_failure(deletion_errors)
            yield (0, 4, self.last_rebuild_result["message"])
            return

        # __init__時に旧形式パスを参照していた場合も、再構築先は常に現行パスへ戻す。
        self.static_index_path = self.rag_data_dir / "faiss_index_static"
        self.dynamic_index_path = self.rag_data_dir / "faiss_index_dynamic"

        yield (1, 4, "記憶索引を再構築中...")
        memory_result = "記憶索引: 対象なし"
        for current, total, status_message in self.update_memory_index_with_progress():
            memory_result = status_message
            yield (1, 4, f"記憶索引: {status_message}")

        yield (2, 4, "ナレッジ索引を再構築中...")
        knowledge_result = self.update_knowledge_index()
        yield (2, 4, knowledge_result)

        yield (3, 4, "現行ログ索引を再構築中...")
        current_log_result = "現行ログ索引: 対象なし"
        for current, total, status_message in self.update_current_log_index_with_progress():
            current_log_result = status_message
            yield (3, 4, f"現行ログ索引: {status_message}")

        verification_errors = self.validate_existing_index_models()
        failure_markers = ("失敗", "エラー", "中止", "中断", "日次上限", "途中保存")
        result_messages = [memory_result, knowledge_result, current_log_result]
        for result_message in result_messages:
            if any(marker in result_message for marker in failure_markers):
                verification_errors.append(result_message)

        expected_paths = []
        if "件を追加保存" in memory_result and not memory_result.startswith("記憶索引: 0件"):
            expected_paths.append(("記憶索引", self.static_index_path))
        if "ファイルを更新" in knowledge_result:
            expected_paths.append(("ナレッジ索引", self.dynamic_index_path))
        if "索引化完了" in current_log_result:
            expected_paths.append(("現行ログ索引", self.rag_data_dir / "current_log_index"))
        for label, path in expected_paths:
            if not path.exists():
                verification_errors.append(f"{label}: 作成完了の報告後に索引実体が見つかりません")

        if verification_errors:
            self._set_rebuild_failure(verification_errors)
            yield (4, 4, self.last_rebuild_result["message"])
            return

        current_model_id = self._get_embedding_model_id()
        RAGManager._model_mismatch_notices = {
            signature
            for signature in RAGManager._model_mismatch_notices
            if not signature.startswith(f"{self.room_name}|")
        }
        message = f"完全再構築が完了しました（使用モデル: {current_model_id}）"
        self.last_rebuild_result = {
            "success": True,
            "message": message,
            "errors": [],
            ...
            "results": {
                ...
                "current_log": current_log_result,
            },
        }
        yield (4, 4, message)

    def rebuild_all_indices(self, status_callback=None) -> str:
        """後方互換用ラッパー。失敗時は例外にして成功扱いを防ぐ。"""
        for _current, _total, message in self.rebuild_all_indices_with_progress():
            if status_callback:
                status_callback(message)
        if not self.last_rebuild_result.get("success"):
            raise RuntimeError(self.last_rebuild_result.get("message", "完全再構築に失敗しました。"))
        return self.last_rebuild_result["message"]
