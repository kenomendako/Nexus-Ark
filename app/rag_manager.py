...
class RotatingEmbeddings(Embeddings):
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        result = self.manager._get_actual_embeddings().embed_query(text)
...
class RAGManager:
    ...
    def _get_actual_embeddings(self):
        ...
        if self.embedding_mode == "local":
            try:
                ...
                self.actual_embeddings = HuggingFaceEmbeddings(model_name=hf_model_id)
    ...
    def _rotate_api_key(self, error_str: str) -> bool:
        ...
        if is_per_minute and not is_daily_limit:
            # キーは切り替えず、指定された時間（+1秒）待機して同じキーでリトライ
            wait = min(retry_delay + 1, 65) if retry_delay else 40
            print(f"      [RAG Rotation] PerMinute limit on '{key_name}'. Waiting {wait:.1f}s for backoff...")
            time.sleep(wait)
            return True
        ...
        if key_name != "Unknown":
            ...
            print(f"      [RAG Rotation] Key '{key_name}' marked as daily-exhausted.")
        ...
        if key_name in paid_keys:
            ...
            if key_name in self.tried_keys:
                self.tried_keys.remove(key_name)
            self.actual_embeddings = None # 再生成を促す
            return True
...
カテゴリ名のみを1単語で回答してください（emotional/factual/technical/temporal/relational）:"""
    ...
    def _safe_save_index(self, db: FAISS, target_path: Path):
        ...
        # 1. システム標準の一時ディレクトリを使用（Windowsのパス長制限 & 非ASCII文字問題を回避）
        # dir=None にすることで OS 標準の一時フォルダが使用される
        with tempfile.TemporaryDirectory(prefix=".tmp_index_") as temp_dir:
    ...
    def _safe_load_index(self, target_path: Path) -> Optional[FAISS]:
        ...
        try:
                ...
                try:
                    ...
                    if saved_model_id and saved_model_id != current_model_id:
                        ...
                        try:
                            import utils
                            utils.add_system_notice(
                                f"RAG警告: 索引のモデル（{saved_model_id}）と現在の設定（{current_model_id}）が不一致です。検索エラーを防ぐため、RAG管理から『完全再構築』を行ってください。",
                                level="warning"
                            )
                        except Exception as e:
            ...
            # ロード実行 (直接読み込み)
            # [Optimization] 1GBを超えるインデックスの場合、copytreeを挟むと10秒以上の遅延が発生するため直接ロードに変更
            db = FAISS.load_local(
                str(target_path),
                embeddings,
