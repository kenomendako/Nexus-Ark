...
SYSTEM_ROOM_DIR_NAMES = {"_shared"}
OLIVIE_ROOM_NAME_ALIASES = {"オリヴェ", "オリベ", "Olivie", "olivie"}

def _infer_month_from_log_backup(backup_path: str) -> Optional[str]:
    """バックアップファイル名または本文の日付から対象月(YYYY-MM)を推定する。"""
    basename = os.path.basename(backup_path)
    filename_match = re.search(r"(\d{4}-\d{2})\.txt", basename)
    if filename_match:
        return filename_match.group(1)

    try:
        # 巨大ログでも起動時の負荷を抑えるため、末尾側だけ読む。通常ログは各発言末尾に日付を持つ。
        with open(backup_path, "rb") as f:
            f.seek(0, os.SEEK_END)
            size = f.tell()
            f.seek(max(0, size - 1024 * 1024))
            sample = f.read().decode("utf-8", errors="ignore")
        date_matches = re.findall(r"(\d{4})[-/](\d{2})[-/]\d{2}", sample)
        if date_matches:
            year, month = date_matches[-1]
            return f"{year}-{month}"
    except Exception as e:
        print(f"--- [REPAIR_LOG] バックアップ月推定に失敗: {backup_path} ({e}) ---")

    try:
        return datetime.datetime.fromtimestamp(os.path.getmtime(backup_path)).strftime("%Y-%m")
    except OSError:
        return None

def restore_missing_monthly_logs_from_backups(room_name: str) -> List[str]:
    """
    backups/logs のバックアップから、欠落または空の月次ログだけを自動復元する。
    既に非空の月次ログがある場合は上書きしない。
    """
    if not room_name or ".." in room_name or "/" in room_name or "\\" in room_name:
        return []

    base_path = os.path.abspath(os.path.join(constants.ROOMS_DIR, room_name))
    return _restore_missing_monthly_logs_from_backups(base_path)

def _restore_missing_monthly_logs_from_backups(abs_base: str) -> List[str]:
    logs_dir = os.path.join(abs_base, constants.LOGS_DIR_NAME)
    backup_logs_dir = os.path.join(abs_base, "backups", "logs")
    restored: List[str] = []

    if not os.path.isdir(backup_logs_dir):
        return restored

    os.makedirs(logs_dir, exist_ok=True)
    latest_backup_by_month: Dict[str, str] = {}

    for name in os.listdir(backup_logs_dir):
        backup_path = os.path.join(backup_logs_dir, name)
        if not os.path.isfile(backup_path) or os.path.getsize(backup_path) == 0:
            continue
        if not (name.endswith(".bak") or name.endswith(".txt")):
            continue

        month = _infer_month_from_log_backup(backup_path)
        if not month or not re.match(r"^\d{4}-\d{2}$", month):
            continue

        current = latest_backup_by_month.get(month)
        if current is None or os.path.getmtime(backup_path) > os.path.getmtime(current):
            latest_backup_by_month[month] = backup_path

    for month, backup_path in sorted(latest_backup_by_month.items()):
        ...
        if os.path.exists(target_path):
            ...
            empty_backup_path = f"{target_path}.empty_before_restore_{timestamp}.bak"
            try:
        ...
        print(f"--- [REPAIR_LOG] 月次ログをバックアップから自動復元: {os.path.basename(backup_path)} -> {target_path} ---")
...
def is_system_room_dir(folder_name: str) -> bool:
    """ユーザーが切り替える通常ルームではない管理用ディレクトリかを判定する。"""
    return bool(folder_name) and (folder_name.startswith(".") or folder_name in SYSTEM_ROOM_DIR_NAMES)

def normalize_room_folder_for_migration(folder_name: str) -> str:
    """旧環境からの移行時に、既知の標準ペルソナ表記ゆれを現在のフォルダ名へ寄せる。"""
    if not isinstance(folder_name, str):
        return folder_name
    import unicodedata

    normalized_name = unicodedata.normalize("NFC", folder_name)
    if normalized_name in OLIVIE_ROOM_NAME_ALIASES:
        return "Olivie"
    return folder_name

def normalize_migrated_config_room_references(config: dict, room_name_map: Dict[str, str]) -> bool:
    """
    移行時にフォルダ名を正規化した場合、config.json内のルーム参照も同じ名前へ更新する。
    戻り値は変更有無。
    """
    if not isinstance(config, dict) or not room_name_map:
        return False

    changed = False

    def mapped_room(value):
        if not isinstance(value, str):
            return value
        return room_name_map.get(value, normalize_room_folder_for_migration(value))

    for key in ("last_room", "line_bot_linked_room"):
        old_value = config.get(key)
        new_value = mapped_room(old_value)
        if new_value != old_value:
            config[key] = new_value
            changed = True

    discord_settings = config.get("discord_bot_settings")
    if isinstance(discord_settings, dict):
        old_value = discord_settings.get("linked_room")
        new_value = mapped_room(old_value)
        if new_value != old_value:
            discord_settings["linked_room"] = new_value
            changed = True

    return changed

def is_auto_generated_default_room(folder_name: str = "Default") -> bool:
    """
    初回起動時のフォールバックで作られた、実質空のDefaultルームかを判定する。
    ユーザーが編集したDefaultはサンプル補填の対象外にするため、主要ファイルが空かだけを見る。
    """
    ...
    if config.get("room_name", folder_name) != folder_name:
        return False
    if config.get("description"):
        return False
    if config.get("agent_display_name"):
        return False

    content_files = [
        os.path.join(room_path, "SystemPrompt.txt"),
        os.path.join(room_path, constants.NOTEPAD_FILENAME),
        os.path.join(room_path, "memory", constants.IDENTITY_FILENAME),
        os.path.join(room_path, "memory", constants.DIARY_FILENAME),
    ]
    for file_path in content_files:
        if os.path.isfile(file_path) and os.path.getsize(file_path) > 0:
            return False

    logs_dir = os.path.join(room_path, constants.LOGS_DIR_NAME)
    if os.path.isdir(logs_dir):
        for name in os.listdir(logs_dir):
            log_path = os.path.join(logs_dir, name)
            if name.endswith(".txt") and os.path.isfile(log_path) and os.path.getsize(log_path) > 0:
                return False

    return True

def should_install_sample_persona_on_startup(sample_room_name: str = "Olivie") -> bool:
    """
    新規配布環境に同梱される管理用データ（例: characters/_shared）だけで、
    サンプルペルソナ展開がスキップされないようにする。
    """
    sample_path = os.path.join(constants.ROOMS_DIR, sample_room_name)
    if os.path.exists(sample_path):
        return False

    room_folders = [folder for _display, folder in get_room_list_for_ui() if not is_system_room_dir(folder)]
    if not room_folders:
        return True

    if room_folders == ["Default"] and is_auto_generated_default_room("Default"):
        return True
...
def ensure_room_files(room_name: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            l_exists = os.path.exists(logs_dir)
            b_exists = os.path.exists(backup_logs_dir)
            
            if l_exists and b_exists:
                # logsが空かチェック
                c_logs = [f for f in os.listdir(logs_dir) if f.endswith(".txt")]
                leg_log = os.path.join(abs_base, "log.txt")
                mig_log = leg_log + ".migrating"
                
                has_legacy_log = os.path.exists(leg_log)
                has_migrating_log = os.path.exists(mig_log)
                needs_repair = not c_logs and (has_legacy_log or has_migrating_log or b_exists)
                if needs_repair:
                    print(f"--- [REPAIR_LOG] 状態確認: {room_name} (logs_dir={len(c_logs)}件, legacy={has_legacy_log}) ---")

                if not c_logs and has_legacy_log:
                    print(f"--- [REPAIR_LOG] legacy logを検知。月次ログへ移行します: {room_name} ---")
                    try:
                        import utils
                        utils.invalidate_log_migration_cache(abs_base)
                        utils._migrate_chat_logs(abs_base)
                        c_logs = [f for f in os.listdir(logs_dir) if f.endswith(".txt")]
                        print(f"--- [REPAIR_LOG] legacy移行後: {room_name} (logs_dir={len(c_logs)}件) ---")
                    except Exception as migrate_error:
                        print(f"--- [REPAIR_LOG] legacy logの月次移行に失敗: {migrate_error} ---")

                restored_monthly_logs = _restore_missing_monthly_logs_from_backups(abs_base)
                if restored_monthly_logs:
                    c_logs = [f for f in os.listdir(logs_dir) if f.endswith(".txt")]
                
                if not c_logs and not has_legacy_log and not has_migrating_log:
                    print(f"--- [REPAIR_LOG] ログ消失を検知。バックアップをスキャンします... ---")
                    
                    b_files = [f for f in os.listdir(backup_logs_dir) if os.path.isfile(os.path.join(backup_logs_dir, f))]
                    print(f"--- [REPAIR_LOG] バックアップ候補: {len(b_files)} 件 ---")
                    
                    backups = sorted(
                        [os.path.join(backup_logs_dir, f) for f in b_files], 
                    ...
                    if backups:
                        ...
                        target_name = "log.txt"
                        # 月次形式 (YYYY-MM.txt) を優先
                        m = re.search(r'(\d{4}-\d{2}\.txt)', fname)
                        if m:
                            target_name = m.group(1)
                        elif "log" in fname.lower():
                            target_name = "log.txt"
                        
                        target_path = os.path.join(logs_dir if target_name != "log.txt" else abs_base, target_name)
                        print(f"--- [REPAIR_LOG] 復元実行: {fname} -> {target_path} ---")
                        shutil.copy2(latest, target_path)
                        if target_name == "log.txt":
                            try:
                                import utils
                                utils.invalidate_log_migration_cache(abs_base)
                                utils._migrate_chat_logs(abs_base)
                            except Exception as migrate_error:
                                print(f"--- [REPAIR_LOG] 復元ログの月次移行に失敗: {migrate_error} ---")
                    else:
                        print(f"--- [REPAIR_LOG] バックアップファイル自体が見つかりませんでした ---")
            elif b_exists:
                print(f"--- [REPAIR_LOG] パスが見つかりません: logs={l_exists}, backups={b_exists} ({backup_logs_dir}) ---")
        except Exception as e:
            print(f"--- [REPAIR_LOG] 重大なエラー: {e} ---")
            traceback.print_exc()
...
def get_room_list_for_ui() -> List[Tuple[str, str]]:
    ...
    for folder_name in os.listdir(rooms_dir):
        # ドット/アンダースコアで始まる管理用フォルダは通常ルームとして扱わない
        if is_system_room_dir(folder_name):
            continue
...
def create_backup(room_name: str, file_type: str) -> Optional[str]:
    ...
    try:
        ...
        # 変更検知: 最新のバックアップと内容が同一ならスキップ
        existing_backups = sorted(
            [f for f in os.listdir(backup_dir) if f.endswith(".bak")],
            key=lambda f: os.path.getmtime(os.path.join(backup_dir, f))
        )
        if existing_backups:
            latest_backup_path = os.path.join(backup_dir, existing_backups[-1])
            try:
                if filecmp.cmp(source_path, latest_backup_path, shallow=False):
                    return None  # 変更なし → スキップ
            except OSError:
                pass  # 比較失敗時はバックアップを作成する
        ...
        # ローテーション処理（ログは専用の世代数を使用）
        if file_type == 'log':
            rotation_count = config_manager.CONFIG_GLOBAL.get("log_backup_rotation_count", 30)
        else:
            rotation_count = config_manager.CONFIG_GLOBAL.get("backup_rotation_count", 10)

        # バックアップ一覧を再取得（新規作成分を含む）
        existing_backups = sorted(
            [f for f in os.listdir(backup_dir) if f.endswith(".bak")],
            key=lambda f: os.path.getmtime(os.path.join(backup_dir, f))
        )
...
def update_room_override_key(room_name: str, key: str, value: Any) -> bool:
    """ルーム個別設定の指定キーだけを差分保存する。"""
    return update_room_config(room_name, {key: value})

def update_room_override_nested(room_name: str, parent_key: str, updates: Dict[str, Any]) -> bool:
    """ルーム個別設定のネスト辞書に対して、指定された子キーだけを差分マージ保存する。"""
    if not isinstance(updates, dict):
        return False
    return update_room_config(room_name, {parent_key: updates})
...
_current_active_room: Optional[str] = None
_periodic_backup_interval: int = 3600 * 3  # デフォルト3時間

def set_active_room_for_backup(room_name: Optional[str]):
    """バックアップ対象のアクティブルームを設定する"""
    global _current_active_room
    _current_active_room = room_name

def _periodic_backup_worker():
    """バックグラウンドで定期的にログバックアップを取得するワーカー"""
    import config_manager
    global _periodic_backup_interval
    while True:
        try:
            # 常に最新の設定から間隔を取得
            interval_val = config_manager.CONFIG_GLOBAL.get("periodic_backup_interval", 10800)
            try:
                _periodic_backup_interval = int(interval_val)
            except ValueError:
                _periodic_backup_interval = 10800

            if _periodic_backup_interval <= 0:
                time.sleep(60)  # 無効化されている場合は1分待機して再確認
                continue

            time.sleep(_periodic_backup_interval)
            
            room = _current_active_room
            if room:
                result = create_backup(room, 'log')
                if result:
                    print(f"--- [定期バックアップ] {room} のログをバックアップしました: {result} ---")
        except Exception as e:
            print(f"警告: 定期バックアップスレッドでエラーが発生しました: {e}")
            time.sleep(60)

def start_periodic_backup():
    """定期バックアップスレッドを開始する"""
    t = threading.Thread(target=_periodic_backup_worker, daemon=True, name="PeriodicLogBackup")
    t.start()
    print("--- 定期ログバックアップスレッドを開始しました ---")

def list_log_backups(room_name: str) -> List[Tuple[str, str]]:
    """ログバックアップの一覧を [(表示名, ファイルパス), ...] の形式で降順（新しい順）で返す"""
    if not room_name:
        return []

    backup_dir = os.path.join(constants.ROOMS_DIR, room_name, "backups", "logs")
    if not os.path.exists(backup_dir):
        return []

    backups = []
    for f in os.listdir(backup_dir):
        if f.endswith("_log.txt.bak"):
            filepath = os.path.join(backup_dir, f)
            # ファイル名からタイムスタンプを抽出 (YYYYMMDD_HHMMSS)
            match = re.match(r"^(\d{8}_\d{6})_log\.txt\.bak$", f)
            if match:
                dt_str = match.group(1)
                try:
                    dt = datetime.datetime.strptime(dt_str, "%Y%m%d_%H%M%S")
                    display_name = dt.strftime("%Y-%m-%d %H:%M:%S")
                    backups.append((display_name, filepath))
                except ValueError:
                    backups.append((f, filepath))
            else:
                backups.append((f, filepath))
...
def restore_log_from_backup(room_name: str, backup_path: str) -> bool:
    """指定されたバックアップからログを復元する。復元前に現在のログのバックアップを取る。"""
    if not room_name or not backup_path or not os.path.exists(backup_path):
        return False
    ...
    log_path_current = current_paths[0]
    
    try:
        # 安全策: 復元する前に現在の状態を強制バックアップ（ファイル名を変えて退避）
        if os.path.exists(log_path_current):
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            pre_restore_backup = f"{log_path_current}.pre_restore_{timestamp}.bak"
            shutil.copy2(log_path_current, pre_restore_backup)
            print(f"--- 復元前の安全バックアップを作成しました: {pre_restore_backup} ---")

        # 復元の実行
        shutil.copy2(backup_path, log_path_current)
        print(f"--- ログをバックアップから復元しました: {backup_path} -> {log_path_current} ---")
        return True
    except Exception as e:
        print(f"致命的エラー: ログの復元に失敗しました: {e}")
        traceback.print_exc()
        return False
