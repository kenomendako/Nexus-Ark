...
def append_action_log(
    ...
    """
    try:
        trigger = str(trigger or "chat").strip().lower()
        if trigger not in {"chat", "autonomous", "scheduled", "system_proxy"}:
            trigger = "chat"
        ...
        # Procedure/Skill系ツールが出す観測用イベントを抽出する。
        skill_event = None
        skill_event_match = re.search(r'^\[skill_event\]\s*(\{.*\})\s*$', summary_result, flags=re.MULTILINE)
        if skill_event_match:
            try:
                parsed_event = json.loads(skill_event_match.group(1))
                if parsed_event.get("type") == "skill_event":
                    skill_event = parsed_event
            except Exception:
                skill_event = None
            summary_result = re.sub(r'\n?\[skill_event\]\s*\{.*\}\s*', '', summary_result, flags=re.MULTILINE).strip()
        ...
        thread_id = safe_args.pop('thread_id', None)
        target_heading = safe_args.pop('target_heading', None)
        ...
        if skill_event:
            record["skill_event"] = skill_event
        ...
        try:
            ...
            tool_usage_stats.record_usage(room_name, tool_name, trigger, status=status)
        except Exception as stats_error:
            print(f"  - [ActionLogger] tool_usage_stats更新をスキップしました: {stats_error}")
    ...
    try:
        ...
        for rec in records:
            ...
            if tool == "web_search_tool":
                ...
                action_desc = f"Web閲覧"
            elif tool in ("update_working_memory", "switch_working_memory", "patch_working_memory", "link_working_memory_to_research_thread"):
            ...
            elif tool == "list_procedures":
                action_desc = "Skill一覧確認"
            elif tool == "read_procedure":
                event = rec.get("skill_event") or {}
                procedure_id = event.get("procedure_id") or rec.get("args", {}).get("procedure_id", "")
                action_desc = f"Skill読込 ({procedure_id})" if procedure_id else "Skill読込"
            elif tool == "save_procedure":
                event = rec.get("skill_event") or {}
                procedure_id = event.get("procedure_id") or rec.get("args", {}).get("procedure_id", "")
                action_desc = f"Skill保存 ({procedure_id})" if procedure_id else "Skill保存"
            elif tool == "create_procedure_from_timeline":
                event = rec.get("skill_event") or {}
                procedure_id = event.get("procedure_id") or rec.get("args", {}).get("procedure_id", "")
                action_desc = f"Skill生成 ({procedure_id})" if procedure_id else "Skill生成"
            ...
            if rec.get("thread_id"):
                prefix += f"[thread:{rec.get('thread_id')}] "
            elif rec.get("target_heading"):
                prefix += f"[heading:{rec.get('target_heading')}] "
