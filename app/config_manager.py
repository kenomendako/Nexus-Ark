...
def set_active_provider(provider: str):
    """アクティブなプロバイダを切り替える"""
    if provider in ["google", "openai", "zhipu", "anthropic"]:
...
def get_internal_model_settings() -> Dict[str, Any]:
    ...
    default_settings = {
        ...
        "translation_provider": "google", # 思考ログ翻訳用
        "translation_model": constants.INTERNAL_PROCESSING_MODEL, # 思考ログ翻訳用
        # OpenAI互換設定
...
def get_effective_internal_model(role: str) -> Tuple[str, str]:
    ...
    # 設定から取得。未設定の場合は内部モデル設定全体の"provider"キーを参照
    # ※ この"provider"はグローバルのactive_providerとは連動しない（internal_model_settings固有の値）
    ...
    # プロバイダとモデル名の不整合を検出（例: provider=openai なのに model=gemini-xxx）
    if provider == "openai" and model_name.startswith("gemini"):
        print(f"  [WARNING] 内部処理モデルの設定に不整合: provider={provider}, model={model_name}, role={role}")
        print(f"  [WARNING] internal_model_settings を確認してください。Google モデルを使用する場合は provider を 'google' に設定してください。")
