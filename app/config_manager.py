...
DEFAULT_MODEL_GLOBAL = "gemini-3-flash-preview"
...
def load_gemini_key_states():
    ...
    [2026-02-11 FIX] GEMINI_KEY_STATESを上書きするのではなくマージし、
    期限切れの枯渇状態は読み込み時にクリアする。
    """
    ...
    if os.path.exists(KEY_STATES_FILE):
        try:
            ...
            for key_name_raw, state in saved_states.items():
                key_name = _clean_api_key_name(key_name_raw) # クレンジング
                if key_name not in GEMINI_API_KEYS: # GEMINI_API_KEYSに存在しないキーは無視
                    continue
                
                # GEMINI_KEY_STATESにまだ存在しない場合は初期化
                if key_name not in GEMINI_KEY_STATES:
                    GEMINI_KEY_STATES[key_name] = {'exhausted': False, 'exhausted_at': 0}

                if state.get('exhausted'):
                    exhausted_at = state.get('exhausted_at', 0)
                    elapsed = now - exhausted_at
                    # 有料キー: 枯渇マーク対象外、無料キー: 3分で自動復帰
                    threshold = 0 if key_name in paid_keys else 180
                    
                    if elapsed > threshold:
                        print(f"--- [API Key Rotation] Key '{key_name}' auto-recovered on load (elapsed: {elapsed:.0f}s) ---")
                        recovered_count += 1
                        continue  # デフォルト（非枯渇）のまま
                
                GEMINI_KEY_STATES[key_name] = state
                if state.get('exhausted'):
                    loaded_count += 1
            
            if loaded_count > 0 or recovered_count > 0:
                print(f"--- [API Key Rotation] Loaded states: {loaded_count} exhausted, {recovered_count} auto-recovered ---")
...
def get_active_gemini_api_key(room_name: str = None) -> Optional[str]:
    """
    指定されたルームの設定（またはグローバル設定）に基づいて、
    現在有効な Gemini API キーの『値（文字列）』を直接返す。
    キーが設定されていない場合は None を返す。
    """
    # [2026-02-11 FIX] 入力をクレンジング
    # (ここでは関数内部で取得するキー名に対して適用する)
    ...
    # まずユーザーが明示的に選択したキーを確認し、枯渇時のみフォールバック
    ...
    if key_name:
        ...
        if key_val and not key_val.startswith("YOUR_API_KEY"):
            if not is_key_exhausted(key_name):
                ...
                alt_key = get_next_available_gemini_key(current_exhausted_key=key_name)
                if alt_key:
...
def is_key_exhausted(key_name: str) -> bool:
    ...
    [2026-02-11 FIX] 有料キーは枯渇マークされないので常にFalse。
    無料キーは3分のクールダウン後に自動復帰する。
    """
    ...
    exhausted_at = state.get('exhausted_at', 0)
    if time.time() - exhausted_at > 180:  # 3分
        print(f"--- [API Key Rotation] Key '{key_name}' auto-recovered (3分経過) ---")
        GEMINI_KEY_STATES[key_name]['exhausted'] = False
        save_gemini_key_states()
        return False
...
def mark_key_as_exhausted(key_name: str):
    ...
    save_gemini_key_states()
    print(f"--- [API Key Rotation] Key '{key_name}' marked as EXHAUSTED (3min cooldown) ---")
