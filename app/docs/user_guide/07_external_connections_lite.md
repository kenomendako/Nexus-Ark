...
チャット入力欄の「🎁」から、所持品をペルソナへ贈る、ユーザーが使う、場所へ配置する、拾う、場所のアイテムを使う操作ができます。実行前に変化内容を確認し、実行後はその出来事をチャットへ伝えられます。
...
「API Gateway / Lite」→「利用可能なAPI一覧・外部連携リファレンス」には、現在実装されている全APIと利用条件が自動表示されます。「安全」はアトリエアプリから追加承認なし、「要承認」は表示された権限をアプリごとに許可した後、「所有者Token限定」は本人の接続用Tokenだけで利用できます。
