...
try:
    ...
    print(default)
' "config.json" "$section" "$key" "$default_value" 2>/dev/null || printf '%s\n' "$default_value"
}

start_tailscale_atelier_https() {
    if [ "${NEXUS_ARK_START_ATELIER_TAILSCALE_SERVE:-0}" != "1" ]; then
        return
    fi

    if ! command -v tailscale &> /dev/null; then
        echo -e "${YELLOW}[WARN] tailscale command not found. Skipping Atelier HTTPS serve.${NC}"
        return
    fi

    local atelier_port
    local atelier_https_port
    atelier_port="${NEXUS_ARK_ATELIER_SERVE_PORT:-$(read_config_value atelier_serve_settings port 8765)}"
    atelier_https_port="${NEXUS_ARK_ATELIER_SERVE_HTTPS_PORT:-$(read_config_value atelier_serve_settings tailscale_https_port 8443)}"
    export NEXUS_ARK_ATELIER_SERVE_ENABLED="${NEXUS_ARK_ATELIER_SERVE_ENABLED:-1}"
    export NEXUS_ARK_ATELIER_SERVE_PORT="$atelier_port"

    local target_url="http://127.0.0.1:${atelier_port}"
    local status_log="/tmp/nexus_ark_tailscale_atelier_serve_status.log"
    local serve_log="/tmp/nexus_ark_tailscale_atelier_serve.log"
    local dns_name
    dns_name="$(timeout 5s tailscale status --json 2>/dev/null | .venv/bin/python -c 'import json,sys; data=json.load(sys.stdin); print((data.get("Self") or {}).get("DNSName","").rstrip("."))' 2>/dev/null || true)"

    if timeout 8s tailscale serve status >"$status_log" 2>&1 && grep -q "$target_url" "$status_log"; then
        echo -e "${GREEN}[OK] Tailscale HTTPS serve for Atelier is already configured.${NC}"
        if [ -n "$dns_name" ]; then
            echo -e "${YELLOW}Atelier HTTPS: https://${dns_name}:${atelier_https_port}/atelier/<room>/<app>/${NC}"
        else
            echo -e "${YELLOW}Atelier HTTPS: https://<your-device>.<tailnet>.ts.net:${atelier_https_port}/atelier/<room>/<app>/${NC}"
        fi
        return
    fi

    echo -e "${YELLOW}[INFO] Configuring Tailscale HTTPS for Atelier apps...${NC}"
    if timeout 20s tailscale serve --bg --https="$atelier_https_port" "$target_url" >"$serve_log" 2>&1; then
        echo -e "${GREEN}[OK] Tailscale HTTPS serve for Atelier configured.${NC}"
        if [ -n "$dns_name" ]; then
            echo -e "${YELLOW}Atelier HTTPS: https://${dns_name}:${atelier_https_port}/atelier/<room>/<app>/${NC}"
        else
            echo -e "${YELLOW}Atelier HTTPS: https://<your-device>.<tailnet>.ts.net:${atelier_https_port}/atelier/<room>/<app>/${NC}"
        ...
        local serve_exit=$?
        if [ $serve_exit -eq 124 ]; then
            echo -e "${YELLOW}[WARN] Atelier Tailscale HTTPS serve setup timed out. Continuing Nexus Ark startup.${NC}"
        else
            echo -e "${YELLOW}[WARN] Atelier Tailscale HTTPS serve setup did not complete.${NC}"
        fi
        echo -e "${YELLOW}       Check: tailscale serve status${NC}"
        if [ -s "$serve_log" ]; then
            sed 's/^/       /' "$serve_log"
...
while true; do
    ...
    if [ "${NEXUS_ARK_ATELIER_SERVE_ENABLED:-0}" = "1" ] || [ "${NEXUS_ARK_START_ATELIER_TAILSCALE_SERVE:-0}" = "1" ]; then
        atelier_port_for_display="${NEXUS_ARK_ATELIER_SERVE_PORT:-$(read_config_value atelier_serve_settings port 8765)}"
        echo -e "${YELLOW}Atelier apps: http://127.0.0.1:${atelier_port_for_display}/atelier/<room>/<app>/${NC}"
    fi
    echo -e "${GREEN}---------------------------------------------------${NC}"
