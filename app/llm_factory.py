...
class LLMFactory:
    ...
    def create_chat_model_with_fallback(
        ...
        **kwargs
        ...
        if internal_role and internal_role in provider_key_map:
            ...
            primary_provider, _ = config_manager.get_effective_internal_model(internal_role)
        else:
    ...
    def invoke_internal_llm(
        ...
        for attempt in range(max_retries):
            # A. 枯渇チェック (Googleのみ)
            if is_google and config_manager.is_key_exhausted(current_key_name):
                print(f"  [LLM Factory Rotation] Key '{current_key_name}' is exhausted. Swapping...")
            ...
            try:
                ...
                if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str or "TOO_MANY_REQUESTS" in err_str:
                    if is_google:
                        print(f"  [LLM Factory Rotation] 429 Error with key '{current_key_name}'.")
                ...
                # 503/502/504: 一時的なサーバーエラー
                elif any(code in err_str for code in ["502", "503", "504"]) or "CONNECTION ERROR" in err_str or "UNAVAILABLE" in err_str:
                    if is_google:
                        print(f"  [LLM Factory Rotation] Server/Connection error (503系) with key '{current_key_name}'. Details: {str(e)[:100]}")
