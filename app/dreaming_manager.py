...
"""
    ...
    def _invoke_llm(self, role: str, prompt: str, settings: dict) -> Any:
        """
        LLMFactory.invoke_internal_llm を使用して LLM を呼び出すラッパー。
        """
        response, used_api_key = LLMFactory.invoke_internal_llm(
            internal_role=role,
            prompt=prompt,
        ...
        # dreaming_manager 内部の状態（api_key）を更新
        self.api_key = used_api_key
        return response, used_api_key