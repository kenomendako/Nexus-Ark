...
class DreamingManager:
    ...
    def _content_to_text(self, content: Any) -> str:
        """LLM応答contentを文字列化する。テスト時にutilsがMockでも安全に処理する。"""
        try:
            text = utils.extract_text_from_llm_content(content)
    ...
    def _build_existing_entity_names_section(self) -> str:
        try:
            em_manager = EntityMemoryManager(self.room_name)
            names = em_manager.list_active_entries()
        except Exception as e:
            print(f"  - [Dreaming] 既存エンティティ名一覧の取得に失敗: {e}")
            return ""

        if not names:
            return ""

        displayed_names = names[:ENTITY_NAME_PROMPT_LIMIT]
        if len(names) > ENTITY_NAME_PROMPT_LIMIT:
            print(
                "  - [Dreaming] 既存エンティティ名一覧が上限を超えたため、"
                f"{ENTITY_NAME_PROMPT_LIMIT}/{len(names)}件だけプロンプトへ注入します。"
            )

        names_text = "\n".join(f"- {name}" for name in displayed_names)
        return f"""
        【既存エンティティ名一覧（新規作成前に必ず照合）】
        {names_text}

        【エンティティ名の統合ルール（最優先）】
        - 新しいエンティティ名を作る前に、上の既存名に同一・近縁・表記ゆれがないか必ず確認してください。
        - 該当する既存名があれば、表記ゆれや言い換えを作らず、既存名をそのまま `entity_name` に使ってください。
        - 同一人物・同一テーマの側面違い（例: 「○○の目標」「○○の恥じらい」）は新規エンティティにせず、その人物・テーマの既存エントリへの追記にしてください。
        """
    ...
    def dream(self, reflection_level: int = 1, skip_consolidation: bool = False, skip_goal_update: bool = False) -> str:
        ...
        try:
            ...
            raw_query_content = self._content_to_text(search_query_msg.content)
        ...
        purpose_profile_text = ""
        try:
            from purpose_profile_manager import PurposeProfileManager
            purpose_profile_text = PurposeProfileManager(self.room_name).get_summary_for_prompt(max_items=5)
            if purpose_profile_text:
                print("  - [Dreaming] Purpose Profileを省察コンテキストに追加しました。")
        except Exception as pe:
            print(f"  - [Dreaming] Purpose Profile読み込みエラー: {pe}")
        ...
        existing_entity_names_section = self._build_existing_entity_names_section()
        ...
        {f"【あなたのPurpose Profile】" + chr(10) + purpose_profile_text if purpose_profile_text else ""}
        {level_specific_instructions}
        {existing_entity_names_section}
        ...
        try:
            ...
            response = self._content_to_text(raw_content)
            ...
            if entity_updates and should_update_entity:
                ...
                for update in entity_updates:
                    ...
                    if e_name and e_content:
                        resolution = em_manager.resolve_entity_candidate(e_name, e_content)
                        if resolution.get("decision") in ("same", "related"):
                            print(
                                "  - [Dreaming] エンティティ候補 "
                                f"'{e_name}' は既存 '{resolution.get('target_name')}' へ "
                                f"{resolution.get('decision')} 判定 "
                                f"(confidence={resolution.get('confidence')}, reason={resolution.get('reason')})"
                            )
                        elif resolution.get("decision") == "reject":
                            print(f"  - [Dreaming] エンティティ候補 '{e_name}' は保存対象外: {resolution.get('reason')}")
            ...
            if reflection_level >= 2 and should_update_entity:
                ...
                em_manager = EntityMemoryManager(self.room_name)
                dormant_names = em_manager.mark_dormant_candidates()
                print(f"  - [Entity Maintenance] 休眠化候補を{len(dormant_names)}件自動休眠化しました。")
                em_manager.consolidate_all_entities(self.api_key)
                try:
                    em_manager.detect_embedding_merge_candidates(self.api_key)
                except Exception as e:
                    print(f"  - [Entity Dedup] ⚠️ embedding重複候補検出をスキップしました: {e}")
                try:
                    em_manager.auto_merge_high_confidence_candidates(self.api_key)
                except Exception as e:
                    print(f"  - [Entity Auto Merge] ⚠️ 高確信候補の自動マージをスキップしました: {e}")
        ...
        # --- [Motivation] 解決済み質問の記憶変換（Phase B） ---
            ...
            # --- [Purpose Profile] 睡眠時整理 ---
            try:
                from motivation_manager import MotivationManager
                from purpose_profile_manager import PurposeProfileManager
                mm = MotivationManager(self.room_name)
                motivation_questions = mm.get_internal_state().get("drives", {}).get("curiosity", {}).get("open_questions", [])
                profile = PurposeProfileManager(self.room_name).consolidate_from_sleep(
                    dream_open_questions=open_questions,
                    motivation_questions=motivation_questions,
                    reflection_level=reflection_level,
                    reason=f"dreaming_level_{reflection_level}",
                )
                print(
                    "  - [Dreaming] Purpose Profileを睡眠時整理しました "
                    f"(active_interests={len(profile.get('active_interests', []))}, "
                    f"open_questions={len(profile.get('open_questions', []))})"
                )
            except Exception as pe:
                print(f"  - [Dreaming] Purpose Profile睡眠時整理エラー: {pe}")
    ...
    def _convert_resolved_questions_to_memory(self, mm, recent_context: str, effective_settings: dict) -> int:
        ...
        converted_count = 0
        for q in questions:
            topic, context = q.get("topic", ""), q.get("context", "")
            answer_summary = q.get("answer_summary", "")
            learned_insight = q.get("learned_insight", "")
            ...
            if not is_substantive_reflection(answer_summary) and not is_substantive_reflection(learned_insight):
                print(f"    → 問い「{topic[:20]}...」は回答詳細が薄いため記憶変換を保留")
                continue
            
            existing_entity_names_section = self._build_existing_entity_names_section()
            prompt = f"以下の「問い」と「回答」から、FACT/INSIGHT/SKIPに分類して抽出してください。\n\n{existing_entity_names_section}\n【問い】{topic}\n【背景】{context}\n【回答要約】{answer_summary}\n【得られた気づき】{learned_insight}\n\n出力形式: JSON"
            try:
                response_msg, current_api_key = self._invoke_llm("summarization", prompt, effective_settings)
                self.api_key = current_api_key
                result = self._parse_json_robust(self._content_to_text(response_msg.content))
                ...
                if c_type == "FACT" and e_name and content:
                    ...
                    # 夢日記は「本物の夢」専用とし、解決問いはエピソード記憶に一本化
                    self._create_discovery_episode(topic, content)
                    print(f"    → {topic[:20]}... を INSIGHT としてエピソード記憶に保存")
                    mm.mark_question_converted(topic)
                    converted_count += 1
                elif c_type == "SKIP":
                    mm.mark_question_converted(topic)
    ...
    def _create_discovery_episode(self, topic: str, content: str):
        ...
        if not is_substantive_reflection(content, min_chars=40):
            print(f"    - 発見エピソードは内容が薄いため生成をスキップ: {topic[:30]}...")
            return
        try:
            epm = EpisodicMemoryManager(self.room_name)
    ...
    def _extract_entity_candidates(self, log_text: str, existing_entities: list) -> list:
        ...
        prompt = (
            "あなたは情報抽出の専門家です。以下の会話からエンティティを抽出してください。\n\n"
            "【重要】出力する各候補について、既存候補に統合すべきかを必ず明示してください。\n"
            "既存候補と同一・近似・別名・親子関係なら、is_new を false にし、"
            "merge_hint に既存候補名や統合理由を入れてください。\n\n"
            f"【会話ログ】\n{log_text[:3000]}\n\n"
            f"【既存】\n{existing_str}\n\n"
            "出力形式: JSON配列 "
            "[{'name': '...', 'is_new': true, 'facts': [...], 'merge_hint': '...'}]"
        )
        try:
            response_msg, current_api_key = self._invoke_llm("processing", prompt, effective_settings)
            self.api_key = current_api_key
            content_text = self._content_to_text(response_msg.content)
            candidates = self._parse_json_robust(content_text) or []
            ...
            for candidate in candidates:
                if not isinstance(candidate, dict):
                    if isinstance(candidate, str):
                        candidate = {"name": candidate, "facts": []}
                    else:
                        continue
                ...
                if not isinstance(facts, list):
                    facts = [str(facts)]
                content = "\n".join([str(f) for f in facts if f]) or str(candidate.get("content", ""))
                resolution = em_manager.resolve_entity_candidate(name, content)
                should_integrate = resolution.get("decision") in {"same", "related", "parent_child"}
                if should_integrate:
                    candidate["is_new"] = False
                    candidate["merge_hint"] = candidate.get("merge_hint") or (
                        f"既存候補 '{resolution.get('target_name')}' と {resolution.get('decision')} 判定"
                    )
                    candidate["existing_target_id"] = resolution.get("target_id")
                    candidate["existing_target_name"] = resolution.get("target_name")
                    candidate["existing_confidence"] = resolution.get("confidence")
                else:
                    candidate["is_new"] = bool(candidate.get("is_new", True))
                    candidate.setdefault("merge_hint", "")
    ...
    def _format_entity_proposal(self, candidates: list) -> str:
        ...
        for c in candidates:
            ...
            merge_hint = c.get("merge_hint") or ""
            if c.get("is_new") is False and merge_hint:
                p.append(f"- 既存候補への統合候補: {merge_hint}")
            elif merge_hint:
                p.append(f"- 統合ヒント: {merge_hint}")
