        ...
        for attempt in range(max_retries):
            ...
            if is_google and config_manager.is_key_exhausted(current_key_name):
                ...
                if next_key:
                    ...
                    print(f"  [Dreaming Rotation] No more available keys. Tried {len(tried_keys)} keys.")
            ...
            try:
                ...
                if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str or "TOO_MANY_REQUESTS" in err_str:
                    if is_google:
                        ...
                        print(f"  [Dreaming] Rate limit error (429) for non-Google provider. Retrying...")
                    time.sleep(2 * (attempt+1)) # バックオフ
                    continue
                elif "502" in err_str or "503" in err_str or "504" in err_str or "CONNECTION ERROR" in err_str:
                    if is_google:
                        print(f"  [Dreaming Rotation] Server/Connection error with key '{current_key_name}'. Details: {str(e)[:100]}")
                        print(f"  [Dreaming Rotation] Swapping key temporarily for retry...")
                        ...
                        print(f"  [Dreaming] Server/Connection error. Details: {str(e)[:100]}")
                        print(f"  [Dreaming] Retrying... ({attempt+1}/{max_retries})")
