...
class PlanNotepadEditArgs(BaseModel):
    modification_request: str = Field(..., description="保存したい内容（文章）そのもの。あなたの書いた文章がそのままメモとして記録されます。")
    room_name: str = Field(..., description="対象のルーム名")

@tool(args_schema=PlanNotepadEditArgs)
...
def _apply_notepad_edits(instructions: List[Dict[str, Any]], room_name: str) -> str:
    ...
    try:
        ...
        for inst in instructions:
            ...
            # バリデーション: contentに実質な内容がない、または'None'の場合はスキップ
            if (not content_stripped or content_stripped == "None") and op != "delete":
