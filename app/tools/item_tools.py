...
def consume_item(item_id: str, room_name: str) -> str:
    ...
    try:
        ...
        if success:
            ...
            res += "\nこれらの情報を元に、アイテムを使った感想や演出をユーザーに伝えてください。"
            return res
        else:
...
def place_item_to_location(item_id: str, location_name: str, room_name: str, furniture_name: str = "", amount: int = 1) -> str:
    ...
    try:
        ...
        if success:
            ...
            return f"成功: アイテム「{item_name}」を {amount} 個、{place_str}に置きました。これは共有アイテムとなり、ユーザーも拾ったり使ったりできます。"
        else:
...
def gift_item_to_user(item_id: str, amount: int, room_name: str) -> str:
    ...
    try:
        ...
        if success:
            ...
            return f"成功: アイテム 「{item_name}」 (ID: {item_id}) を {amount} 個、ユーザーに贈りました。\nユーザーには「アイテム（名称・カテゴリ）」のみが伝わっています。ユーザーがチャット入力欄の下にある「アイテム使用（添付・消費）」メニューからこのアイテムを消費すると、詳細な味が伝わります。そのことをチャットでユーザーに伝えてください。"
        else:
...
def create_food_item(name: str, category: str, amount: int, base_description: str, room_name: str, image_prompt: Optional[str] = None, description: Optional[str] = None) -> str:
    ...
    ※ユーザーから「プレゼントされたもの」を食べる場合は、新しく作成するのではなく list_my_items で既存のアイテムを探して consume_item を実行してください。
    name: アイテム名。
    ...
    try:
        ...
        if success:
            ...
            res += "\n作成したアイテムをすぐにユーザーに贈りたい場合は、この ID を使って gift_item_to_user を呼び出せます。\nlist_my_items で一覧を確認することも可能です。"
            return res
        else:
...
def create_standard_item(name: str, category: str, amount: int, base_description: str, room_name: str, image_prompt: Optional[str] = None, description: Optional[str] = None) -> str:
    ...
    ※ユーザーから「プレゼントされたもの」を身につけたり使用する場合は、新しく作成するのではなく list_my_items で既存のアイテムを探して examine_item 等を実行してください。
    name: アイテム名。
    ...
    try:
        ...
        if success:
            ...
            res += "\nこれを場所に置いたり(place_item_to_location)、ユーザーに贈ったり(gift_item_to_user)することが可能です。"
            return res
        else:
