...
from file_lock_utils import locked_file, safe_json_read, safe_json_write, safe_text_read, safe_text_write
...
def _save_wm_metadata(room_name: str, metadata: dict) -> None:
    ...
    safe_json_write(_get_wm_metadata_path(room_name), metadata)
...
def update_working_memory(content: str, room_name: str, context_type: str = "CONTINUE", intent: str = "情報の更新", slot_name: str = None) -> str:
    ...
    try:
        ...
        safe_text_write(path, _normalize_working_memory_text(content).rstrip() + "\n")
