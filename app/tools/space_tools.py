...
@tool
def list_available_locations(room_name: str) -> str:
    """
    現在移動可能な世界内の「エリア」と「場所」の一覧を取得します。
    別の場所に移動（set_current_location）する前に、どのような場所があるか確認するために使用します。
    """
    if not room_name:
    ...
    for area, places in world_data.items():
        if isinstance(places, dict):
            valid_places = [p for p in places.keys() if not p.startswith("__")]
            if valid_places:
                lines.append(f"■ {area}")
    ...
    if not lines:
        return "【情報】現在、移動可能な場所は定義されていません。"

    result = "【移動可能なエリアと場所の一覧】\n" + "\n".join(lines)
    result += "\n\n※移動するには set_current_location の location_id に、上の場所名をそのまま指定してください。"    
    return result
...
def set_current_location(location_id: str, room_name: str) -> str:
    ...
    # 2c. ファジーマッチ（部分一致・英語名推測への対応）
    if not corrected_location_id:
        input_lower = original_input.lower().replace('_', ' ').replace('-', ' ')
        best_match = None
        best_score = 0
        for loc in valid_locations:
            loc_lower = loc.lower()
            # 場所名が入力に含まれている（例: 「植物園」→「硝子張りの植物園」）
            if loc_lower in input_lower or input_lower in loc_lower:
                score = len(loc)  # 長い方が具体的な一致
                if score > best_score:
                    best_match = loc
                    best_score = score
        if best_match:
            corrected_location_id = best_match
    ...
    if corrected_location_id:
        try:
            ...
            # 補正が行われた場合はその旨を伝える
            correction_note = ""
            if corrected_location_id != original_input:
                correction_note = f"（入力 '{original_input}' → '{corrected_location_id}' に補正しました）\n"
            
            return (f"Success: 現在地は '{corrected_location_id}' に設定されました。{correction_note}"
                    f"**以下の【移動後の情景】が、今のあなたの目に映る光景です。**\n\n"
                    f"【移動後の情景】\n{scene_description}\n\n"
