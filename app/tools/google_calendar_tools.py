...
プライバシーフィルタはグローバル既定（privacy_filter_default）を適用する。
ルーム個別の表示カレンダー絞り込み・フィルタ上書きは後続フェーズで配線する。
"""
...
def _visible_events(start: datetime.datetime, end: datetime.datetime) -> List[Dict[str, Any]]:
    """範囲内のイベントを取得し、グローバル既定のプライバシーフィルタを適用して返す。"""
    settings = gcal._get_settings()
    cache = gcal.load_cache()
    selected = settings.get("selected_calendars") or None
    events = gcal.get_events_in_range(cache, start, end, calendar_ids=selected)
    return gcal.apply_privacy_filter(events, settings.get("privacy_filter_default"))
...
@tool
def read_calendar_schedule(date: str = "today", days: int = 1) -> str:
    """
    ...
    events = _visible_events(range_start, range_end)
    if not events:
        if days == 1:
...
@tool
def check_free_time(date: str = "today", start_time: str = "00:00", end_time: str = "23:59") -> str:
    """
