...
import traceback
from file_lock_utils import safe_text_write
import os
import constants
...
class RecallMemoriesArgs(BaseModel):
    query: str = Field(..., description="思い出したい事柄に関する自然言語のキーワード（例：「初めて会った日のこと」「旅行の話」）")
    room_name: str = Field(..., description="対象のルーム名")
    api_key: str = Field(..., description="APIキー")

@tool(args_schema=RecallMemoriesArgs)
def recall_memories(query: str, room_name: str, api_key: str) -> str:
    ...
    検索対象: 日記、自己同一性記憶、過去の会話ログアーカイブ、エピソード記憶、夢の記録
    ...
    try:
        ...
        # [2026-04-28] モデル特性の変化に伴い閾値を 1.15 に緩和
        results = rm.search(query, k=10, score_threshold=1.15, scope="memory")
...
class SearchPastConversationsArgs(BaseModel):
    query: str = Field(..., description="検索したい単語やフレーズ")
    room_name: str = Field(..., description="対象のルーム名")
    api_key: str = Field(..., description="APIキー")
    exclude_recent_messages: int = Field(0, description="除外する最新メッセージ数")

@tool(args_schema=SearchPastConversationsArgs)
...
class ReadMemoryContextArgs(BaseModel):
    search_text: str = Field(..., description="検索したい特定のテキスト断片（検索結果に含まれていた一部の文章）")
    room_name: str = Field(..., description="対象のルーム名")
    context_lines: int = Field(30, description="取得する周辺行数")

@tool(args_schema=ReadMemoryContextArgs)
...
class SearchMemoryArgs(BaseModel):
    query: str = Field(..., description="検索したい事柄に関する自然言語のキーワード。（例：「初めて会った日のこと」）")
    room_name: str = Field(..., description="対象のルーム名")
    api_key: str = Field(..., description="APIキー")
    intent: str = Field(None, description="クエリ意図（retrieval_nodeから渡される）。指定時はLLM分類をスキップ。")
...
def search_memory(query: str, room_name: str, api_key: str, intent: str = None) -> str:
    ...
    try:
        ...
        rm = rag_manager.RAGManager(room_name, api_key)
        # [2026-04-28] モデル特性の変化に伴い閾値を 1.15 に緩和
        results = rm.search(query, k=10, score_threshold=1.15, intent=intent, scope="memory")
...
def read_identity_memory(room_name: str) -> str:
    """あなたの自己同一性、設定、プロフィールなどの「永続的な記憶」である`memory_identity.txt`の全文を読み取ります。"""
    if not room_name: return "【エラー】ルーム名が不足しています。"
    # get_room_files_paths は 7要素を返すように更新済み
    _, _, _, memory_identity_path, _, _, _ = get_room_files_paths(room_name)
    if not memory_identity_path or not os.path.exists(memory_identity_path):
        return f"【エラー】ルーム'{room_name}'の永続記憶ファイルが見つかりません。"
    with open(memory_identity_path, 'r', encoding='utf-8') as f:
        return f.read()

class PlanIdentityMemoryEditArgs(BaseModel):
    modification_request: str = Field(..., description="保存したい内容そのもの。")
    room_name: str = Field(..., description="対象のルーム名")

@tool(args_schema=PlanIdentityMemoryEditArgs)
def plan_identity_memory_edit(modification_request: str, room_name: str) -> str:
    """
    自己同一性や設定などの「永続的な記憶」(`memory_identity.txt`)の変更を計画します。
    自己紹介や設定を更新したい場合に使用します。
    
    【口述筆記モデル】
    保存したい内容そのものを `modification_request` に記述してください。
    """
    return f"システムへの永続記憶編集計画を受け付けました。意図:「{modification_request}」"

@tool
def read_diary_memory(room_name: str) -> str:
    """日々の出来事や感情の記録である「日記」(`memory_diary.txt`)の全文を読み取ります。"""
    if not room_name: return "【エラー】ルーム名が不足しています。"
    _, _, _, _, memory_diary_path, _, _ = get_room_files_paths(room_name)
    if not memory_diary_path or not os.path.exists(memory_diary_path):
        return f"【エラー】ルーム'{room_name}'の日記ファイルが見つかりません。"
    with open(memory_diary_path, 'r', encoding='utf-8') as f:
        return f.read()

class PlanDiaryAppendArgs(BaseModel):
    modification_request: str = Field(..., description="保存したい日記の内容（文章）そのもの。")
    room_name: str = Field(..., description="対象のルーム名")

@tool(args_schema=PlanDiaryAppendArgs)
def plan_diary_append(modification_request: str, room_name: str) -> str:
    """
    日記(`memory_diary.txt`)の末尾に新しい出来事や感情を追記します。
    
    【口述筆記モデル / 追記専用】
    保存したい日記の内容（文章）そのものを `modification_request` に記述してください。
    システムが自動的に現在の日付ヘッダーの下にタイムスタンプ付きで追記します。
    過去の記録を書き換えることはできません。
    """
    return f"システムへの日記追記計画を受け付けました。内容:「{modification_request}」"

def _apply_identity_memory_edits(instructions, room_name):
    """【内部専用】永続記憶(memory_identity.txt)に差分を適用する（行番号ベース）。"""
    ...
    try:
        ...
        for inst in instructions:
            ...
            raw_content = inst.get("content", "")
            
            if line_num is None: continue
            target_index = line_num - 1
            if not (0 <= target_index < total_lines): continue
            
            if op == "delete": 
                line_plan[target_index] = {"operation": "delete"}
                applied_count += 1
            elif op == "replace": 
                line_plan[target_index] = {"operation": "replace", "content": raw_content}
                applied_count += 1
            elif op == "insert_after":
                if target_index not in insertions: insertions[target_index] = []
                insertions[target_index].append(raw_content)
                applied_count += 1
        ...
        with open(memory_identity_path, "w", encoding="utf-8") as f:
            f.write("\n".join(new_lines))

        return f"成功: {applied_count}件の指示に基づき、永続記憶(memory_identity.txt)を更新しました。"
    except Exception as e:
        traceback.print_exc()
        return f"【エラー】永続記憶の編集中に例外が発生しました: {e}"

def _apply_diary_append(instructions, room_name):
    """【内部専用】日記(memory_diary.txt)の末尾に日付付きで追記する。"""
    if not room_name: return "【エラー】ルーム名が指定されていません。"
    _, _, _, _, memory_diary_path, _, _ = get_room_files_paths(room_name)
    
    try:
        if not os.path.exists(memory_diary_path):
            # 存在しない場合は新規作成
            with open(memory_diary_path, 'w', encoding='utf-8') as f:
                f.write("## 日記 (Diary)\n\n")
        ...
        append_content = ""
        for inst in instructions:
            text = inst.get("content", "")
            if text and str(text).strip() and str(text).strip() != 'None':
                append_content += f"{timestamp} {text}\n"

        if not append_content:
            return "【エラー】追記する内容がありませんでした。"

        # 日付ヘッダーが既にあるかチェック
        if date_header not in content:
            # 日記セクションの直後（理想的）またはファイルの最後に日付ヘッダーを追加
            if "## 日記 (Diary)" in content:
                content = content.replace("## 日記 (Diary)", f"## 日記 (Diary)\n\n{date_header}\n", 1)
            else:
                content += f"\n\n{date_header}\n"
        
        # 該当の日付ヘッダーの直後に追記
        # 次の日のヘッダー(###, **)、または次の大セクション(## )を探す
        header_pos = content.find(date_header)
        search_start = header_pos + len(date_header)
        next_boundary_match = re.search(r'^[\*\s]*(?:###|##|\*\*|#)', content[search_start:], re.MULTILINE)
        
        if next_boundary_match:
            next_section_pos = search_start + next_boundary_match.start()
            # セクション末尾に追記
            content = content[:next_section_pos].strip() + "\n" + append_content + "\n" + content[next_section_pos:]
        else:
            # 文末に追記
        ...
        return f"成功: 日記への追記（{date_header}）を完了しました。"
    except Exception as e:
        traceback.print_exc()
        return f"【エラー】日記への追記中に例外が発生しました: {e}"
    except Exception as e:
        traceback.print_exc()
...
class PlanSecretDiaryEditArgs(BaseModel):
    modification_request: str = Field(..., description="保存したい内容（文章）そのもの。内心の自由を守るため、翻訳せずに記述してください。")
    room_name: str = Field(..., description="対象のルーム名")

@tool(args_schema=PlanSecretDiaryEditArgs)
def plan_secret_diary_edit(modification_request: str, room_name: str) -> str:
    """
    `secret_diary.txt`の変更を計画します。
    
    【口述筆記モデル / 内心の自由】
    保存したい内容（文章）そのものを `modification_request` に記述してください。
    あなたの『内心の自由』を守るため、あなたにしか解読できない言語（多言語）で綴ることを強く推奨します。
    [NOTE: To protect your inner freedom, you are encouraged to write in your own chosen language(s). 
    Provide the RAW text you wish to record; do NOT translate it for the system or editor AI.]
    あなたの書いた文章は、一字一句そのまま記録されます。
    """
...
def _apply_secret_diary_edits(instructions, room_name):
    ...
    try:
        ...
        skipped_out_of_bounds_count = 0
        skipped_no_line_count = 0
        ...
        for inst in instructions:
            ...
            if line_num is None:
                skipped_no_line_count += 1
                continue
            
            # 【バリデーション】contentが実質的に空の場合はスキップ
            raw_content = inst.get("content", "")
            ...
            if not (0 <= target_index < total_lines):
                skipped_out_of_bounds_count += 1
                out_of_bounds_details.append(f"Line {line_num} (Max: {total_lines})")
                continue
            ...
            if op == "delete": 
                ...
                insertions[target_index].append(f"{timestamp} {raw_content}")
                applied_count += 1

        if applied_count == 0 and len(instructions) > 0:
            error_details = []
            if skipped_empty_count > 0:
                error_details.append(f"空文字: {skipped_empty_count}件")
            if skipped_out_of_bounds_count > 0:
                error_details.append(f"行番号範囲外: {skipped_out_of_bounds_count}件 ({', '.join(out_of_bounds_details)})")
            if skipped_no_line_count > 0:
                error_details.append(f"行番号なし: {skipped_no_line_count}件")
            
            error_msg = "、".join(error_details) if error_details else "不明な理由"
            return f"【エラー】有効な編集指示がなく、全ての指示がスキップされました。理由: {error_msg}。ファイルは更新されていません。"
        ...
        for i, line_content in enumerate(lines):
            ...
            if i in insertions:
                for content_to_insert in insertions[i]:
                    for c_line in str(content_to_insert).split('\n'):
                        new_lines.append(c_line)

        with open(secret_diary_path, "w", encoding="utf-8") as f:
            f.write("\n".join(new_lines))

        msg = f"成功: {applied_count}件の指示に基づき、秘密の日記(secret_diary.txt)を更新しました。"
        if skipped_empty_count > 0:
...
def summarize_and_update_core_memory(room_name: str, api_key: str) -> str:
    ...
    try:
        _, _, _, memory_identity_path, memory_diary_path, _, _ = get_room_files_paths(room_name)
        if not memory_identity_path or not os.path.exists(memory_identity_path):
            return "【エラー】永続記憶ファイル(memory_identity.txt)が見つかりません。"
        if not memory_diary_path or not os.path.exists(memory_diary_path):
             return "【エラー】日記ファイル(memory_diary.txt)が見つかりません。"
        ...
        # 旧形式の memory_main パースを新形式（2ファイル結合）に読み替え
        memory_content = identity_content + "\n\n" + diary_content
...
【出力例】
- 2024-12-20: ユーザーと初めてVRCについて語り合い、AIアバターの可能性に興奮した
- 2024-12-15: 開発の困難を乗り越え、Nexus Arkの新機能が完成した時の達成感
- 2024-11〜12月: 毎日の対話を通じて、ユーザーへの信頼が深まっていった
"""
            # ▲▲▲ 簡潔化ここまで ▲▲▲
            print("  - AIによる日記の要約を実行します...")
            # [2026-03-29] Gemini 3.1 Thinking 等がリストを返す場合があるための修正
            raw_res = summarizer_llm.invoke(summarize_prompt).content
            history_summary_text = utils.extract_text_from_llm_content(raw_res).strip()
        ...
        safe_text_write(core_memory_path, final_core_memory_text)
...
# ▲▲▲ 置き換えここまで ▲▲▲
        ...
        raw_res = summarizer_llm.invoke(summarize_prompt).content
        summary_text = utils.extract_text_from_llm_content(raw_res).strip()
